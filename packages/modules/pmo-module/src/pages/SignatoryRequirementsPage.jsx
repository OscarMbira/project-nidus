...
export default function SignatoryRequirementsPage() {
  ...
  useEffect(() => {
    ...
    platformDb
      ...
      .eq('menu_code', 'plat_tpl_signatory_requirements')
      .maybeSingle()
      .then(({ data }) => {
        if (cancelled) return
  ...
  const refreshDocCounts = async () => {
    ...
    const result = await getSignatoryCountsForDocumentTables(platformDb, {
      accountId,
      scopeType,
  ...
  useEffect(() => {
    ...
    let cancelled = false
    setLoading(true)
    getScopedSignatoryConfig(platformDb, {
      accountId,
      scopeType,
      scopeId: needsEntity ? scopeId : null,
      ...
      .then((result) => {
        ...
        if (mode === 'custom' || mode === 'organisation') {
          ...
          // inherit — do not resolve parent slots here (that was hanging Loading…);
          // parent list is applied at document signing. Custom copies parent on demand.
          setSlots([])
          setSourceBanner(
  ...
  const applyMode = async (nextMode) => {
    ...
    const effective = await resolveEffectiveSignatoryRequirements(platformDb, {
      accountId,
      documentTable: primaryTable,
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    const result = await saveSignatoryRequirementsForTables(platformDb, {
      accountId,
  ...
  return (
    <div className="mx-auto flex max-w-7xl flex-col gap-4 p-6 lg:h-[calc(100dvh-6.5rem)] lg:min-h-0 lg:overflow-hidden">
      <div className="shrink-0">
        <Link to="/app/pmo/organisational-templates" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
