...
/** Fill-in form lives on the Project Documents detail route (not templates/:id). */
 ...
 * Route entry: /platform/documents/project
 */
export default function ProjectDocumentsRegisterPage() {
  ...
  const [viewMode, setViewMode] = useViewMode('project-documents-register', 'list')

  useEffect(() => {
    getMenuLabel(platformDb, 'plat_pm_project_documents', 'Project Documents').then(setPageTitle)
  }, [])
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const result = await loadProjectDocumentsRegister(platformDb, {
        accountId,
        projectId,
        schema: 'public',
      })
  ...
  const handleCaptureOrRestore = async (row) => {
    ...
    try {
      ...
      const { node, mode } = await captureOrRestoreProjectDocument(platformDb, {
        accountId,
        projectId,
        sourceNode: row,
      ...
      // Navigate immediately to the fill-in form (don't wait for register reload).
  ...
  const handleRetire = async (row) => {
    ...
    setBusyId(row.id)
    try {
      await retireProjectDocument(platformDb, row)
