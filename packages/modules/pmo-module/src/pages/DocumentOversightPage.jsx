...
const DETAIL_LIST_BASE = '/platform/documents/project'

const TIER_META = {
  portfolio: { menuCode: 'plat_pmo_portfolio_document_oversight', defaultTitle: 'Document Oversight', empty: 'You are not currently the manager of any portfolio, so there are no documents to show.' },
  programme: { menuCode: 'plat_pmo_programme_document_oversight', defaultTitle: 'Document Oversight', empty: 'You are not currently the manager of any programme, so there are no documents to show.' },
  pmo: { menuCode: 'plat_pmo_document_oversight', defaultTitle: 'Document Oversight', empty: 'No project documents with a signatory requirement exist yet.' },
}
 ...
 * out to the existing Project Documents detail page.
...
export default function DocumentOversightPage({ tier }) {
  ...
  const [viewMode, setViewMode] = useViewMode(`document-oversight-${tier}`, 'list')

  useEffect(() => {
    getMenuLabel(platformDb, meta.menuCode, meta.defaultTitle).then(setPageTitle)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const scopeResult = await resolveOversightProjectScope(platformDb, { tier, schema: 'public' })
      if (!scopeResult.success) throw new Error(scopeResult.message)
      const docsResult = await listOversightDocuments(platformDb, {
        accountId,
