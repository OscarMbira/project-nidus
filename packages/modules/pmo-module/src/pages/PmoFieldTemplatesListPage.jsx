...
/**
 * PMO Field Template Manager — lists domain=fields nodes for the current account.
 * Route (when wired): /app/pmo/field-templates
 */
export default function PmoFieldTemplatesListPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const data = await listFieldTemplateNodes(platformDb, accountId)
        if (!cancelled) {
          setRows(data)
          setError(null)
          setLoading(false)
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4 md:p-6">
      <div>
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">PMO Field Templates</h1>
        <p className="text-sm text-gray-500 dark:text-gray-400">
          Hierarchy nodes for record fields (Global sync + PMO customisations). System-synced rows are read-only.
        </p>
      </div>
      ...
      {!loading && !error && filtered.length === 0 && (
        <p className="text-sm text-gray-500">No field template nodes for this account yet. Publish a Global Template from Admin, or create a PMO draft.</p>
      )}
        ...
        {filtered.map((row, index) => (
          ...
          >
            <div>
              ...
              <Link
                to={`/app/pmo/field-templates/${row.id}`}
