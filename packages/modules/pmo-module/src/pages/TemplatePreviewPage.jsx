 ...
 * Route: /app/pmo/template-library/preview/:nodeId
 */
export default function TemplatePreviewPage() {
  ...
  useEffect(() => {
    getMenuLabel(platformDb, 'plat_tpl_library', 'Global Template Library').then(setBackLabel)
  }, [])
    ...
    ;(async () => {
      try {
        ...
        const [contentInfo, accountId] = await Promise.all([
          getNodeContent(platformDb, n),
          getCurrentUserAccountId(),
        ])
        if (cancelled) return
        setInfo(contentInfo)
        if (accountId && ['portfolio', 'programme', 'project'].includes(entityType)) {
          const override = await resolveAccountTemplateOverride(platformDb, { accountId, globalNodeId: n.id })
  ...
  const backHref = `/app/pmo/template-library${searchParams.toString() ? `?${searchParams.toString()}` : ''}`
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      const { node: copied } = await copyTemplateNodeForAccount(platformDb, {
        accountId,
