...
const shellSrc = 'stakeholder-module'.startsWith('sim-') ? simulatorSrc : platformSrc

export default defineConfig({
  plugins: [
    react({ jsxRuntime: 'automatic' }),
    federation({
      name: 'stakeholder_module',
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'stakeholder_module', version: '1.0.0', status: 'ok' }))
        })
        server.middlewares.use('/modules/stakeholder-module/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'stakeholder_module', version: '1.0.0', status: 'ok' }))
        })
      },
