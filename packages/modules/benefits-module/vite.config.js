...
const shellSrc = 'benefits-module'.startsWith('sim-') ? simulatorSrc : platformSrc

export default defineConfig({
  plugins: [
    react({ jsxRuntime: 'automatic' }),
    federation({
      name: 'benefits_module',
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'benefits_module', version: '1.0.0', status: 'ok' }))
        })
        server.middlewares.use('/modules/benefits-module/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'benefits_module', version: '1.0.0', status: 'ok' }))
        })
      },
