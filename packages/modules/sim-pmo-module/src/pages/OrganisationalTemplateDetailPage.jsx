...
export default function OrganisationalTemplateDetailPage() {
  ...
  useEffect(() => {
    if (isProjectDocuments) {
      getMenuLabel(simDb, 'sim_pm_project_documents', 'Project Documents').then(setBackLinkLabel)
    } else if (isProjectList) {
      getMenuLabel(simDb, 'sim_pm_project_templates', 'Project Templates').then(setBackLinkLabel)
    } else {
      getMenuLabel(simDb, 'sim_tpl_organisational', 'Organisational Templates').then(setBackLinkLabel)
    }
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const n = await getTemplateNode(simDb, decodeURIComponent(String(nodeIdOrReference || '')))
      if (!n) {
      ...
      if (n.parent_node_id) {
        try {
          const parent = await getTemplateNode(simDb, n.parent_node_id)
      ...
      const info = await getNodeContent(simDb, n)
      setContentInfo(info)
      if (info.content) {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    // public.users lives on the Platform schema — resolve labels via platformDb even in Simulator.
  ...
  useEffect(() => {
    ...
    resolveScopeReferenceLabel(simDb, { scopeType, scopeId }).then((label) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const nodePatch = fillProjectDoc
        ...
        : form
      await updateTemplateNode(simDb, node.id, nodePatch)
      if (contentInfo.kind === 'opa' && contentInfo.content) {
        await updateOpaContent(simDb, contentInfo.content.id, {
        ...
        await updateProcessTemplateContent(simDb, contentInfo.table, contentInfo.content.id, {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (node.domain === 'process_template') {
        await archiveProcessTemplateNodeAndContent(simDb, node)
      } else {
        await archiveTemplateNode(simDb, node.id)
      }
  ...
  useEffect(() => {
    ...
    const projectIdForResolve =
      (node.scope_entity_type === 'project' && node.scope_entity_id) || contextProjectId || null
    resolveEffectiveSignatoryRequirements(simDb, {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    isDocumentFullySigned(simDb, node.id).then((signed) => {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    resolveDocumentSignaturesForExport(simDb, node.id).then((result) => {
  ...
  useEffect(() => {
    if (!node?.id) return
    let cancelled = false
    resolveDocumentAttachmentsForExport(simDb, node.id).then((result) => {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
      ...
      {contentInfo.kind === 'process_template' && contentInfo.content && node?.id && (
        <DocumentAttachmentsPanel db={simDb} templateNodeId={node.id} mode="sim" disabled={fullySigned || !canManageDocument} />
      )}
