 ...
 * Route: /simulator/pmo/organisational-templates/:nodeId
 */
export default function OrganisationalTemplateDetailPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const n = await getTemplateNode(simDb, nodeIdOrReference)
      if (!n) {
      ...
      if (n.is_system_synced) {
        ...
        navigate('/simulator/pmo/template-library')
        return
      }
      ...
      if (n.template_reference && n.template_reference !== nodeIdOrReference) {
        navigate(`/simulator/pmo/organisational-templates/${n.template_reference}`, { replace: true })
      }
      setNode(n)
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await updateTemplateNode(simDb, node.id, form)
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      navigate('/simulator/pmo/organisational-templates')
    } catch (e) {
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-4 md:p-6">
      ...
      {node.domain === 'fields' && (
        <div className="rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          ...
          <Link
            to={`/simulator/pmo/field-templates/${node.id}`}
      ...
      {contentInfo.kind === 'opa' && (
        <div className="space-y-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            ...
            <Link to={`/simulator/opa/${contentInfo.content?.id}/edit`} className="text-blue-600 hover:underline dark:text-blue-400">
              full OPA editor
            </Link>.
          </p>
        </div>
