...
const shellSrc = 'sim-pmo-module'.startsWith('sim-') ? simulatorSrc : platformSrc
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'sim_pmo_module', version: '1.0.0', status: 'ok' }))
        })
        server.middlewares.use('/modules/sim-pmo-module/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'sim_pmo_module', version: '1.0.0', status: 'ok' }))
        })
      },
