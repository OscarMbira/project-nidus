...
const shellSrc = 'stage-gates-module'.startsWith('sim-') ? simulatorSrc : platformSrc

export default defineConfig({
  plugins: [
    react({ jsxRuntime: 'automatic' }),
    federation({
      name: 'stage_gates_module',
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'stage_gates_module', version: '1.0.0', status: 'ok' }))
        })
        server.middlewares.use('/modules/stage-gates-module/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'stage_gates_module', version: '1.0.0', status: 'ok' }))
        })
      },
