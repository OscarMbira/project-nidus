...
const shellSrc = 'sim-planning-module'.startsWith('sim-') ? simulatorSrc : platformSrc

export default defineConfig({
  plugins: [
    react({ jsxRuntime: 'automatic' }),
    federation({
      name: 'sim_planning_module',
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'sim_planning_module', version: '1.0.0', status: 'ok' }))
        })
        server.middlewares.use('/modules/sim-planning-module/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'sim_planning_module', version: '1.0.0', status: 'ok' }))
        })
      },
