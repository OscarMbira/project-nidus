...
const shellSrc = 'MODULE_FOLDER'.startsWith('sim-') ? simulatorSrc : platformSrc
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'MODULE_NAME', version: 'MODULE_VERSION', status: 'ok' }))
        })
        server.middlewares.use('/modules/MODULE_FOLDER/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'MODULE_NAME', version: 'MODULE_VERSION', status: 'ok' }))
        })
      },
