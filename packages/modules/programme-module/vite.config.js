...
const shellSrc = 'programme-module'.startsWith('sim-') ? simulatorSrc : platformSrc

export default defineConfig({
  plugins: [
    react({ jsxRuntime: 'automatic' }),
    federation({
      name: 'programme_module',
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'programme_module', version: '1.0.0', status: 'ok' }))
        })
        server.middlewares.use('/modules/programme-module/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'programme_module', version: '1.0.0', status: 'ok' }))
        })
      },
