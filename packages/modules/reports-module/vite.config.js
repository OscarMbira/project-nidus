...
const shellSrc = 'reports-module'.startsWith('sim-') ? simulatorSrc : platformSrc
    ...
    }),
    {
      ...
      configureServer(server) {
        server.middlewares.use('/health.json', (_req, res) => {
          ...
          res.end(JSON.stringify({ name: 'reports_module', version: '1.0.0', status: 'ok' }))
        })
        server.middlewares.use('/modules/reports-module/health', (_req, res) => {
          res.setHeader('Content-Type', 'application/json')
          res.end(JSON.stringify({ name: 'reports_module', version: '1.0.0', status: 'ok' }))
        })
      },
