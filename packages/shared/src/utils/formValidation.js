...
/**
 * Check a single rendered schema field against its submitted value.
 * Empty means undefined/null, empty/whitespace-only string, or an empty array.
 */
...
/**
 * Validate every required field in a rendered form schema (post-override, post-merge —
 * whatever schema is actually shown to the user) against submitted values.
 * Returns a map of field.key -> error message for each empty required field ({} if none).
 */
export function validateRequiredSchemaFields(schema, values = {}) {
  const errors = {}
  for (const section of schema?.sections || []) {
    for (const field of section.fields || []) {
      if (!field.required) continue
      if (isRequiredFieldEmpty(values[field.key])) {
