...
import {
  ...
  updateFieldAttachmentCaption,
  uploadFieldAttachment,
  validateAttachmentFile,
  DEFAULT_MAX_FILES_PER_FIELD,
} from '../../services/formFieldAttachmentService'
import { formatFileSize } from '../../services/fileUploadService'

/** Full-size preview modal for one attachment version — signed-URL equivalent of DocumentPreview.jsx,
 *  built locally because DocumentPreview.jsx is coupled to the document-governance download flow. */
function AttachmentPreviewModal({ attachment, mode, onClose }) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    getAttachmentSignedUrl(attachment.storage_path, mode).then((result) => {
      if (cancelled) return
...
function VersionHistoryDropdown({ attachment, mode, onRestore, onPreview, onClose }) {
  const [history, setHistory] = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    let cancelled = false
    listAttachmentVersionHistory(attachment.attachment_group_id, mode).then((result) => {
      if (cancelled) return
      if (result.success) setHistory(result.data)
      setLoading(false)
    })
    return () => { cancelled = true }
  }, [attachment.attachment_group_id, mode])
...
function AttachmentThumbnail({ attachment, mode, disabled, onOpenPreview, onReplace, onDelete }) {
  ...
  useEffect(() => {
    ...
    return () => { cancelled = true }
  }, [attachment.storage_path, mode, isImage])
  ...
  return (
    <div className="relative flex w-40 flex-col gap-1 rounded border border-gray-200 bg-white p-2 dark:border-gray-700 dark:bg-gray-900">
      ...
      <p className="text-[10px] text-gray-400 dark:text-gray-500">
        {attachment.display_id || 'v' + attachment.version_number} · {formatFileSize(attachment.file_size)}
      </p>
      {disabled ? (
        attachment.caption ? (
        ...
        <input
          ...
          onBlur={() => {
            if (captionValue !== (attachment.caption || '')) {
              updateFieldAttachmentCaptionSafe(attachment.attachment_group_id, captionValue, mode)
            }
          }}
...
// Fire-and-forget caption save — errors surface via toast, don't block typing.
async function updateFieldAttachmentCaptionSafe(attachmentGroupId, caption, mode) {
  const result = await updateFieldAttachmentCaption({ attachmentGroupId, caption }, mode)
  if (!result.success) toast.error(result.message || 'Failed to save caption')
}

export default function AttachmentField({ field, formInstanceId, onChange, disabled = false, mode = 'platform' }) {
  ...
  const accept = field.accept === 'image' ? 'image' : 'any'
  const maxFiles = Math.min(Number(field.maxFiles) || DEFAULT_MAX_FILES_PER_FIELD, DEFAULT_MAX_FILES_PER_FIELD)

  const refresh = useCallback(async () => {
    if (!formInstanceId) return
    setLoading(true)
    const result = await listFieldAttachments(formInstanceId, field.key, mode)
    if (result.success) setAttachments(result.data)
    setLoading(false)
  }, [formInstanceId, field.key, mode])
  ...
  const notifyValueChange = useCallback((nextAttachments) => {
    onChange?.(field.key, nextAttachments.map((a) => a.attachment_group_id))
  }, [onChange, field.key])

  const handleUpload = useCallback(async (file) => {
    if (!formInstanceId) return
    const validationError = validateAttachmentFile(file, { accept, maxFiles, currentCount: attachments.length })
    if (validationError) {
    ...
    const result = await uploadFieldAttachment(
      { formInstanceId, fieldKey: field.key, file, uploadedByUserId: userIdRef.current },
      mode,
    )
    setUploading(false)
    if (!result.success) {
    ...
    const next = [...attachments, result.data]
    setAttachments(next)
    notifyValueChange(next)
    toast.success(`Attached ${result.data.display_id || result.data.file_name}`)
  }, [formInstanceId, field.key, accept, maxFiles, attachments, mode, notifyValueChange])
  ...
  const handleReplace = useCallback(async (attachmentGroupId, file, { restoreVersionNumber } = {}) => {
    ...
    const result = restoreVersionNumber
      ? await restoreAttachmentVersion({ attachmentGroupId, versionNumber: restoreVersionNumber, uploadedByUserId: userIdRef.current }, mode)
      : await replaceFieldAttachment({ attachmentGroupId, file, uploadedByUserId: userIdRef.current }, mode)
    setUploading(false)
    if (!result.success) {
      toast.error(result.message || 'Failed to update attachment')
      return
    }
    const next = attachments.map((a) => (a.attachment_group_id === attachmentGroupId ? result.data : a))
    setAttachments(next)
  ...
  const handleDelete = useCallback(async (attachmentGroupId) => {
    ...
    const result = await deleteFieldAttachment({ attachmentGroupId, deletedByUserId: userIdRef.current }, mode)
    if (!result.success) {
      toast.error(result.message || 'Failed to delete attachment')
      return
    }
    const next = attachments.filter((a) => a.attachment_group_id !== attachmentGroupId)
    setAttachments(next)
    notifyValueChange(next)
    toast.success('Attachment deleted')
  }, [attachments, mode, notifyValueChange])
  ...
  if (!formInstanceId) {
    return (
      <p className="text-xs text-gray-500 dark:text-gray-400 italic">
        Setting up this field for attachments…
      </p>
    )
  }
  ...
  return (
    ...
    >
      ...
      {previewTarget && (
        <AttachmentPreviewModal attachment={previewTarget} mode={mode} onClose={() => setPreviewTarget(null)} />
      )}
    </div>
  )
}
