...
export default function EnhancedRiskForm({ risk, projectId, riskRegisterId, onSave, onCancel, variant: variantProp, accountId: accountIdProp = null }) {
  ...
  const handleSubmit = async () => {
    if (risk && isRecordLifecycleLocked(risk.record_status)) {
      return
    }
    if (!validateStep(activeStep)) {
      return
  ...
  return (
    ...
    >
      ...
      <RecordLifecycleFieldLock recordStatus={risk?.record_status}>
        ...
        <div className="p-6">
          {renderStepContent()}
          {risk?.id && accountId && projectId && (
            <InheritedRiskRegisterFields
              db={simDb}
              accountId={accountId}
              projectId={projectId}
              practiceProjectId={projectId}
              riskId={risk.id}
              mode={isRecordLifecycleLocked(risk?.record_status) ? 'view' : 'edit'}
            />
          )}
        </div>
