/**
 * Risk Register Export Menu — single Export control for register PDF + list formats.
 */
...
import { exportRiskRegisterToPDF, exportRiskRegisterToCSV } from '@nidus/shared/utils/riskExport'
import {
  exportToExcel,
  exportListToWord,
  exportListToPPT,
...
export default function RiskExportMenu({
  register,
  risks,
  onPrint,
  columns = [],
  ...
  const listRows = data?.length ? data : (risks || [])
  ...
  const handleExportPDF = async () => {
    ...
    setShowMenu(false)
    try {
      await exportRiskRegisterToPDF(register, risks)
    } catch (error) {
  ...
  const handleExportRegisterCSV = () => {
    ...
    setShowMenu(false)
    try {
      exportRiskRegisterToCSV(register, risks)
    } catch (error) {
      ...
      <div className="relative inline-block" ref={menuRef}>
        ...
        {showMenu && (
          <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-2 z-50">
            ...
            <button type="button" onClick={handleExportPDF} disabled={exporting || !risks?.length} className={itemClass}>
              <FileText className="h-4 w-4" />
              PDF report
            </button>
            <button type="button" onClick={handleExportRegisterCSV} disabled={exporting || !risks?.length} className={itemClass}>
              <File className="h-4 w-4" />
              CSV (full risk fields)
            </button>
            <button type="button" onClick={handlePrint} className={itemClass}>
