...
import { getQualityChecks, updateQualityCheck } from '../../services/checkpointReportQualityService'

export default function CheckpointQualityCriteria({ reportId, qualityStatus, onStatusChange, mode }) {
  ...
  const handleStatusChange = async (checkId, newStatus, overrideReason = null) => {
    try {
      ...
      if (onStatusChange) {
        const { getQualityCheckStatus } = await import('../../services/checkpointReportQualityService')
    ...
    <div className="space-y-6">
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        ...
        <p className="text-sm text-blue-700 dark:text-blue-300">
          Five quality criteria that must be met before submitting the checkpoint report for approval.
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm font-medium mt-2">
                Completion: {qualityStatus.completion_percentage?.toFixed(0) || 0}%
              </p>
            </div>
