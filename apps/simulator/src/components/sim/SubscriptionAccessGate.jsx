...
const SubscriptionAccessGate = ({ children, requiredFeature, requiredRole, scenario = null, requiresCollaborative = false }) => {
  ...
  const checkAccess = async () => {
    try {
      ...
      if (!summary.isActive) {
        setHasAccess(false);
      } else if (requiresCollaborative) {
        // Collaborative mode requires an active Team seat specifically, not
        // just any active subscription — see v747_sim_collaborative_access_
        // gating.sql. The RPCs enforce this server-side too; this only
        // controls whether the page renders or shows the upgrade prompt.
        const { hasActiveTeamSeat } = await import('../../services/sim/simTeamSeatService');
        setHasAccess(await hasActiveTeamSeat(user.id));
      } else if (requiredRole) {
        const tier = summary?.tier || summary?.planTier || 'free';
        const { canAccessRoleForTier } = await import('@nidus/shared/constants/simulatorRoles');
        setHasAccess(canAccessRoleForTier(requiredRole, tier));
      } else if (scenario) {
    ...
    <div className="max-w-2xl mx-auto">
      <div className={`rounded-xl p-8 text-center ${theme === 'dark' ? 'bg-gray-800' : 'bg-white'} shadow-lg`}>
        ...
        {statusSummary?.inGracePeriod ? (
          ...
          <p className={`mb-6 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
            {requiresCollaborative
              ? 'Collaborative Team sessions require an active Team subscription seat.'
              : scenario
