...
import ApprovalChainDisplay from '@nidus/ui/ApprovalChainDisplay'
import AuthorisationRequestModal from '@nidus/ui/AuthorisationRequestModal'
import RecordStatusBadge from '@nidus/ui/RecordStatusBadge'
...
export function AuthorisationQueuePage({ service, pmoView = false, title = 'Authorisation Queue' }) {
  ...
  const openReview = async (row) => {
    ...
    try {
      const data = await service.fetchPendingChanges(row.table_name, row.record_id)
      setPending(data)
    } catch (err) {
      setPendingError(err.message || 'Failed to load pending changes')
    } finally {
