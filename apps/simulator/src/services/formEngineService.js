...
export async function upsertFormTemplate(
  ...
  try {
    ...
    const { data, error } = await db.from('form_templates').insert(payload).select('*').single()
    if (error) throw error
    return ok(data)
  } catch (error) {
    return fail(error)
...
export async function getFormInstance(formInstanceId, mode = 'platform') {
  try {
    ...
    return ok({ ...instance, values: values || [], rows: rows || [], schema, template })
  } catch (error) {
    return fail(error)
  }
}
...
export async function getFieldOverridesForOrg(organisationId, templateId, mode = 'platform', scope = {}) {
  try {
    ...
    let query = getDb(mode)
      ...
      .select('section_key, field_key, is_enabled, is_required, scope_entity_type, scope_entity_id')
...
export async function addFieldForOrg(
  ...
  try {
    ...
    // per-scope unique constraint) — getFormTemplateFieldUsage() checks usage by field_key
...
/**
 * Resolve the ancestor chain (root-to-leaf) for a Practice Portfolio/Programme/Project entity,
 * per the "checked, not assumed" linkage rules — mirrors the Platform version but against the
 * `sim` schema's own table/column names (sim.practice_portfolio_projects/practice_programme_projects
 * use practice_portfolio_id/practice_programme_id/practice_project_id, and sim.practice_programmes'
 * parent column is practice_portfolio_id, not portfolio_id — genuinely different names, not aliases).
  ...
  try {
    ...
    if (entityType === 'project') {
      const [{ data: portfolioLink }, { data: programmeLink }] = await Promise.all([
        db.from('practice_portfolio_projects').select('practice_portfolio_id').eq('practice_project_id', entityId).maybeSingle(),
        db.from('practice_programme_projects').select('practice_programme_id').eq('practice_project_id', entityId).maybeSingle(),
      ])
      if (portfolioLink?.practice_portfolio_id) chain.push({ entityType: 'portfolio', entityId: portfolioLink.practice_portfolio_id })
      if (programmeLink?.practice_programme_id) chain.push({ entityType: 'programme', entityId: programmeLink.practice_programme_id })
      chain.push({ entityType: 'project', entityId })
    } else if (entityType === 'programme') {
      const { data: programme } = await db.from('practice_programmes').select('practice_portfolio_id').eq('id', entityId).maybeSingle()
      if (programme?.practice_portfolio_id) chain.push({ entityType: 'portfolio', entityId: programme.practice_portfolio_id })
