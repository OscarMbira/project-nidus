...
describe('renameMenuItem', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  it('calls rename_menu_item RPC and returns updated row', async () => {
    const row = { id: 'menu-1', menu_label: 'Renamed Item' }
    platformDb.rpc.mockResolvedValue({ data: row, error: null })

    const result = await renameMenuItem('menu-1', 'Renamed Item')

    expect(platformDb.rpc).toHaveBeenCalledWith('rename_menu_item', {
      p_menu_item_id: 'menu-1',
      p_new_label: 'Renamed Item',
    })
    expect(result).toEqual(row)
  })

  it('propagates RPC permission errors', async () => {
    platformDb.rpc.mockResolvedValue({
      data: null,
      error: { message: 'Permission denied: System Admin required' },
    })

    await expect(renameMenuItem('menu-1', 'X')).rejects.toEqual({
      message: 'Permission denied: System Admin required',
    })
  })
})
