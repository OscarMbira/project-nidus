...
export const SUBSCRIPTION_TIERS = {
  ...
  TESTER: {
    ...
    billingCycle: 'tester',
    features: [
      'All scenarios during moratorium',
      'Advanced AI feedback',
...
function resolveSimulatorTier(subscription) {
  if (isTesterSubscription(subscription)) return SUBSCRIPTION_TIERS.TESTER;
  if (!subscription) return SUBSCRIPTION_TIERS.FREE;
  return SUBSCRIPTION_TIERS[subscription.plan_type?.toUpperCase()] || SUBSCRIPTION_TIERS.FREE;
}
...
export async function hasFeatureAccess(userId, feature) {
  try {
    ...
    const tier = resolveSimulatorTier(subscription);
...
/**
 * Schedule simulator subscription cancellation at period end
 */
export async function cancelSimulatorSubscription(subscriptionId) {
  try {
    const { data, error } = await simDb
      .from('simulator_subscriptions')
      .update({
