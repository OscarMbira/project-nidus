...
export async function submitForAuthorisation(tableName, recordId, rootRecordId, notes, proposedChanges = null) {
  const { data: { user } } = await simDb.auth.getUser()
  if (!user) throw new Error('Not authenticated')

  const { data, error } = await simDb.rpc('submit_for_authorisation', {
...
export async function fetchPendingChanges(tableName, recordId) {
  const { data, error } = await simDb.rpc('get_pending_changes', {
    p_table_name: tableName,
    p_record_id: recordId,
  })
  if (error) throw error
...
export async function processDecision(requestId, decision, notes) {
  const { data, error } = await simDb.rpc('process_authoriser_decision', {
