...
/**
 * Resolve the OPA master a practice project should default to, walking the
 * PM Template Hierarchy chain. `sim.practice_projects` has no account/org
 * column (it's user-scoped), so the account context comes from the current
 * user, matching how SimOPAList/OPAList already resolve `organisationId`.
 */
export async function resolveEffectiveOpaMaster(projectId) {
  if (!projectId) return { data: null, error: null }
  try {
    const accountId = await getCurrentUserAccountId()
    const node = await resolveEffectiveDocumentMaster(simDb, 'project', projectId, 'opa', { accountId })
