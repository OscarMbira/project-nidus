...
export async function updatePracticeRisk(riskId, updates) {
  try {
    const userId = await getCurrentUserId()
    const patch = { ...updates, updated_by: userId, updated_at: new Date().toISOString() }
    ...
    const governed = await tryGovernedLifecycleUpdate({
      ...
      currentRow: existing,
      updates: patch,
      notes: 'Practice risk change submitted for authorisation',
      physicalTable: 'practice_risks',
      useDirectConfig: true,
      scopeOptions: { projectTable: 'practice_projects', projectIdField: 'practice_project_id' },
    })
    ...
    const { data, error } = await simDb.from('practice_risks').update(patch).eq('id', riskId).eq('user_id', userId).select().single()
    if (error) throw error
    return { success: true, data }
  } catch (error) {
    return { success: false, error: error.message }
