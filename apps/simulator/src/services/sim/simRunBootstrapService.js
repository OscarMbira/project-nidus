...
import { getNpcRolesForUserRole } from './npcRoleFilterService'
...
export async function assignNPCCharacters(runId, userRole) {
  ...
  const allowedNpcRoles = new Set(getNpcRolesForUserRole(userRole))
  const rows = []
  for (const role of ALL_NPC_ROLES) {
    if (!allowedNpcRoles.has(role)) continue
...
/**
 * @param {{ scenarioId: string, userRole: string, methodology?: string, difficulty?: keyof typeof DIFFICULTY_MOD, collaborativeSessionId?: string }} opts
 * collaborativeSessionId (v736 Phase F): tags this run as one of a
 * Collaborative Team session's 3 participant runs. Each participant calls
 * this independently for themselves — one person cannot create a run row
 * for someone else (RLS: auth.uid() = user_id, same as any solo run) — see
 * start_collaborative_session() in v745, which only flips session status;
 * it doesn't create runs.
 */
export async function startSimulationRun(opts) {
  try {
    const authUserId = await getSimAuthUserId()

    const { data: isEligible, error: eligError } = await simDb.rpc('check_scenario_trial_eligibility', {
      p_user_id: authUserId,
    })
    if (eligError) {
      console.error('check_scenario_trial_eligibility', eligError)
      return { success: false, error: 'Failed to check trial eligibility' }
    }
    if (!isEligible) {
      return {
        success: false,
        error: "You've used both of your free trial scenarios. Upgrade to Professional for unlimited access.",
      }
    }
    ...
    const { data: run, error: runErr } = await simDb
      ...
      .insert({
        ...
        collaborative_session_id: opts.collaborativeSessionId || null,
      })
      .select()
      .single()
