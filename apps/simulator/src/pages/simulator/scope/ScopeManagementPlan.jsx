...
import { simGetScopeManagementPlan, simSaveScopeManagementPlan } from '../../../services/sim/simPlanningService'
...
export default function ScopeManagementPlan() {
  ...
  const load = useCallback(async () => {
    if (!projectId) return
    setLoading(true)
    const res = await simGetScopeManagementPlan(projectId)
  ...
  const save = async (asDraft) => {
    ...
    try {
      ...
      const res = await simSaveScopeManagementPlan(
        projectId,
      ...
      {formTab === 'audit' ? (
        <div className="rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-700 dark:bg-gray-900">
          {!record ? (
            ...
            <AuditDetailsPanel description="When this scope management plan was created and last changed.">
