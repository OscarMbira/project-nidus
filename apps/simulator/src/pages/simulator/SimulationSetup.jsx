...
const ROLES = SIMULATOR_ROLE_LIST.map((r) => r.id)
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const { data } = await simDb
        .from('scenarios')
        .select('id,name,short_description,methodology,difficulty_level,scenario_data,target_role')
        .eq('is_active', true)
        .order('sort_order', { ascending: true })
      const rows = (data || []).filter((s) => !s.target_role || s.target_role === activeRole)
      ...
      {step === 2 && (
        <div>
          <div className="grid gap-2">
            {ROLES.map((r) => {
              const meta = SIMULATOR_ROLE_LIST.find((x) => x.id === r)
              return (
              <button
                key={r}
                type="button"
                onClick={() => setUserRole(r)}
                className={`rounded border px-3 py-2 text-left ${userRole === r ? 'border-blue-500 bg-blue-500/10' : 'border-gray-300 dark:border-gray-600'}`}
              >
