  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (!c) setProvenanceByNodeId(map)
      } catch {
        if (!c) setProvenanceByNodeId(new Map())
      }
    })()
    return () => {
      c = true
          ...
          {displayRows.map((r, index) => (
            <div key={r.id} className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
              ...
              {provenanceByNodeId.has(r.pm_template_node_id) && (
                <Link
                  to={`/simulator/pmo/template-library/preview/${provenanceByNodeId.get(r.pm_template_node_id).id}`}
