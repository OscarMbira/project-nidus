/**
 * Budget Categories Management (PMO Admin)
 * CRUD for budget_categories lookup – used in project financial controls (e.g. Machinery, Labour).
 * Route: /platform/pmo-admin/budget-categories
 */
...
export default function BudgetCategories() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      const result = await getBudgetCategories({ activeOnly: false });
      if (result.success) {
        setItems(result.data || []);
      } else {
        const msg = result?.error || 'Failed to load budget categories';
        toast.error(msg);
      }
    } catch (e) {
      const msg = e?.message || e?.error_description || 'Failed to load budget categories';
      console.error(e);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  const handleDelete = useCallback(async (row) => {
    ...
    try {
      const result = await deleteBudgetCategory(row.id);
      if (result.success) {
  ...
  const handleSubmit = useCallback(async (e) => {
    ...
    try {
      ...
      const result = editing
        ? await updateBudgetCategory(editing.id, data)
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-gray-100">Budget Categories</h1>
              <p className="mt-2 text-gray-400">Manage budget category labels for project financial controls (e.g. Machinery, Labour)</p>
            </div>
            <button
              onClick={handleAdd}
        ...
        {showForm && (
          <div className="bg-gray-800 rounded-lg border border-gray-700 p-6 mb-6">
            <h2 className="text-xl font-semibold text-gray-100 mb-4">
              {editing ? 'Edit Budget Category' : 'Add Budget Category'}
            </h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {formTab === 'audit' && (
                !editing?.id ? (
                  <p className="text-sm text-gray-500 dark:text-gray-400">Audit details appear after this category is saved.</p>
                ) : (
                  <AuditDetailsPanel description="Who created or changed this budget category, and how it is classified.">
                    <AuditCard title="Identity" description="How this category is labelled and tracked.">
                      <AuditField label="Name" value={formData.name || editing.name} />
                      <AuditField label="Code" value={formData.code || editing.code} />
                    </AuditCard>
                    <AuditCard title="Classification" description="Where this category sits.">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Machinery, Labour"
                    required
                  />
                </div>
                <div>
              ...
              {items.length === 0 ? (
                <tr>
                  ...
                  <td colSpan={5} className="px-6 py-8 text-center text-gray-400">
                    No budget categories. Add one or run the seed script to load best-practice categories.
                  </td>
                </tr>
              ) : (
                items.map((row, index) => (
