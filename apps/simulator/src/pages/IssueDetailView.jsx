...
export default function IssueDetailView() {
  ...
  useEffect(() => {
    ...
    if (!projectRef) {
      setIssueAccountId(null)
      return
    }
    let cancelled = false
    // Account lives on public.projects (same pattern as Simulator RiskDetail)
    platformDb
      .from('projects')
      .select('account_id')
      .eq('id', projectRef)
      .maybeSingle()
      .then(({ data }) => {
        if (!cancelled) setIssueAccountId(data?.account_id || null)
      })
    return () => {
      cancelled = true
    }
  ...
  const handleStatusChange = async (newStatus) => {
    if (isRecordLifecycleLocked(issue?.record_status)) return
    try {
      if (newStatus === 'closed') {
  ...
  const lifecycleLocked = isRecordLifecycleLocked(issue.record_status)
        ...
        <div className="flex gap-2 flex-wrap">
          {issue.status === 'awaiting_decision' && (
            <button
              ...
              disabled={lifecycleLocked}
              className="px-3 py-1 bg-yellow-600 hover:bg-yellow-700 text-white rounded text-sm disabled:cursor-not-allowed disabled:opacity-50"
            >
              Record Decision
            </button>
          )}
          {issue.status === 'raised' && (
            <button
              ...
              className="px-3 py-1 bg-blue-600 hover:bg-blue-700 text-white rounded text-sm disabled:cursor-not-allowed disabled:opacity-50"
              disabled={lifecycleLocked}
            >
              Start Assessment
            </button>
          ...
          {issue.status === 'approved' && (
            <button
              ...
              className="px-3 py-1 bg-green-600 hover:bg-green-700 text-white rounded text-sm disabled:cursor-not-allowed disabled:opacity-50"
              disabled={lifecycleLocked}
            >
              Start Resolution
            </button>
          ...
          {issue.status === 'resolved' && (
            <button
              ...
              className="px-3 py-1 bg-gray-600 hover:bg-gray-700 text-white rounded text-sm disabled:cursor-not-allowed disabled:opacity-50"
              disabled={lifecycleLocked}
            >
              Close Issue
            </button>
