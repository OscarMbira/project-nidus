...
/**
 * Practice-project form field parameterization only (Platform parity).
 * Dropdown is limited to form templates copied into Project Templates.
 */
export default function ProjectFieldTemplates() {
  ...
  useEffect(() => {
    getMenuLabel(db, 'sim_pm_project_templates_forms', 'Forms').then(setBackLinkLabel)
  }, [])
  ...
  useEffect(() => {
    ...
    db.from('practice_projects')
      .select('project_name, project_code, practice_code')
      .eq('id', projectId)
      .maybeSingle()
      .then(({ data }) => {
        if (cancelled) return
        ...
        const code =
          ...
          (data?.practice_code != null && String(data.practice_code).trim()) ||
          ''
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400 mb-6">
        Parameterise form templates that have been copied into Project Templates for this practice
        project. Does not change the organisation-wide form master.
      </p>
      {catalogError && (
