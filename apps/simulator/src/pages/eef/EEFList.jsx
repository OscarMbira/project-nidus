...
export default function EEFList() {
  ...
  const [viewMode, setViewMode] = useViewMode('simulator-eef-list', 'list')
    ...
    <div className="w-full px-3 sm:px-4 lg:px-5 xl:px-6 py-6">
      <div className="mb-6 flex flex-col gap-4">
        ...
        {pageTab === 'register' && (
          <div className="flex flex-wrap gap-2 justify-end">
            ...
            <Link
              to="/simulator/eef/bulk-upload"
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium min-h-[44px]"
            >
              Bulk upload
            </Link>
            <Link
              to="/simulator/eef/new"
      ...
      {pageTab === 'dashboard' && (
        <RegisterOpenItemsWidget
          ...
          rowKey={(r) => r.id}
          searchFields={['title']}
          onRowClick={(r) => navigate(`/simulator/eef/${r.id}`)}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayRows.map((r, index) => (
            ...
            >
              ...
              <button type="button" onClick={() => navigate(`/simulator/eef/${r.id}`)} className="text-left font-semibold text-gray-900 dark:text-white hover:underline pr-10">
                {r.title}
              </button>
              ...
              <div className="mt-auto flex gap-2 pt-2">
                <RowActionButton variant="view" label="View EEF" onClick={() => navigate(`/simulator/eef/${r.id}`)} />
                <RowActionButton variant="edit" label="Edit EEF" onClick={() => navigate(`/simulator/eef/${r.id}/edit`)} />
                <RowActionButton
                  variant="delete"
                  label="Delete EEF"
