  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <h1 className="text-xl font-bold text-gray-900 dark:text-gray-100">{template.label}</h1>
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative">
            ...
            {exportOpen && (
              <div className="absolute right-0 mt-1 z-10 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-lg py-1 min-w-[120px]">
                {['csv', 'json', 'xml', 'print'].map((fmt) => (
                  <button
                    key={fmt}
                    type="button"
                    onClick={() => handleExport(fmt)}
                    className="block w-full text-left px-3 py-1.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 uppercase"
                  >
                    {fmt}
                  </button>
                ))}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((row, idx) => (
            <div key={row.id} className="relative rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900/60 p-4">
              <RowNumberBadge number={getDisplayRowNumber(idx, filtered.length)} className="absolute top-3 right-3" />
              <Link to={`${listBase}/${row.id}`} className="font-medium text-gray-900 dark:text-gray-100 hover:text-blue-600 dark:hover:text-blue-400">
                {row.title || 'Untitled'}
              </Link>
        ...
        <div className="overflow-x-auto rounded-xl border border-gray-700">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {filtered.map((row, idx) => (
                <tr key={row.id} className="border-t border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/40">
