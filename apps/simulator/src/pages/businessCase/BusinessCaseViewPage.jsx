...
} from '@nidus/shared/utils/businessCaseExportSections'
import InheritedBusinessCaseFields from '../../features/local-data-extensions/components/InheritedBusinessCaseFields'
...
export default function BusinessCaseViewPage() {
  ...
  useEffect(() => {
    ...
    getCurrentUserAccountId().then((aid) => {
      if (!cancelled && aid) setAccountId(aid)
    })
    if (!projectId) {
    ...
    platformDb
      ...
      .maybeSingle()
      .then(({ data }) => {
        if (!cancelled && data?.account_id) setAccountId(data.account_id)
      })
    return () => { cancelled = true }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {accountId && businessCase.project_id && id && (
        <div className="mt-6 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          <InheritedBusinessCaseFields
            db={simDb}
            accountId={accountId}
            projectId={businessCase.project_id}
            practiceProjectId={businessCase.project_id}
            businessCaseId={id}
            mode="view"
          />
        </div>
