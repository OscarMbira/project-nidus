 ...
 */
export async function saveProgramme(programmeData, programmeId = null, options = {}) {
      ...
      try {
        ...
        if (accountId) {
          const parent = pickCreateParentLink({
            portfolioId: data.portfolio_id || programmeData.portfolio_id || null,
          })
          const result = await applyFieldTemplateInheritanceOnCreate(platformDb, {
            accountId,
            entityType: 'programme',
            entityId: data.id,
            entityName: data.programme_name,
          ...
          if (!result.ok) {
            console.warn('Programme field-template inheritance skipped:', result.error)
          }
        }
      } catch (inhErr) {
        console.warn('Programme field-template inheritance failed (non-fatal):', inhErr)
      }
    }
