...
const PORTFOLIO_COLUMNS = [
  ...
  'portfolio_goals', 'governance_model', 'review_frequency', 'delivery_methodology_track', 'total_budget', 'budget_currency',
 ...
 * Opt-in (create only): options.applyFieldTemplateInheritance — wires pm_template_nodes under parent/PMO.
 */
export async function savePortfolio(portfolioData, portfolioId = null, options = {}) {
  const { data: { user } } = await platformDb.auth.getUser()
  if (!user) throw new Error('User not authenticated')

  const appUserId = await getAppUserId(user.id)
  const applyInheritance = options.applyFieldTemplateInheritance === true
  ...
  if (applyInheritance && data?.id) {
    try {
      const accountId = options.accountId || (await getCurrentUserAccountId())
      if (accountId) {
        const parent = pickCreateParentLink({
          parentPortfolioId: data.parent_portfolio_id || portfolioData.parent_portfolio_id || null,
        })
        const result = await applyFieldTemplateInheritanceOnCreate(platformDb, {
          accountId,
          entityType: 'portfolio',
          entityId: data.id,
          entityName: data.portfolio_name,
        ...
        if (!result.ok) {
          console.warn('Portfolio field-template inheritance skipped:', result.error)
        }
      }
    } catch (inhErr) {
      console.warn('Portfolio field-template inheritance failed (non-fatal):', inhErr)
    }
  }
