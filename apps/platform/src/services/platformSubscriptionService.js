...
export const PLATFORM_SUBSCRIPTION_TIERS = {
  ...
  TESTER: {
    ...
    billingCycle: 'tester',
    features: [
      'Full professional-tier access during moratorium',
      'All project methodologies',
      'Unlimited projects and team members for test runs',
    ],
    limits: {
      max_projects: -1,
      max_team_members: -1,
...
/**
 * True when subscription is an active system-tester moratorium grant.
 */
export function isTesterSubscription(subscription) {
  if (!subscription?.is_tester) return false;
  if (subscription.expires_at && new Date(subscription.expires_at) < new Date()) return false;
  return subscription.status === 'active' || subscription.status === 'trialing';
}

function resolvePlatformTier(subscription) {
  if (isTesterSubscription(subscription)) {
    return PLATFORM_SUBSCRIPTION_TIERS.TESTER;
  }
  if (!subscription) return PLATFORM_SUBSCRIPTION_TIERS.FREE;
  return PLATFORM_SUBSCRIPTION_TIERS[subscription.plan_type?.toUpperCase()] || PLATFORM_SUBSCRIPTION_TIERS.FREE;
}
...
export async function hasPlatformFeatureAccess(userId, feature) {
  try {
    ...
    const tier = resolvePlatformTier(subscription);
