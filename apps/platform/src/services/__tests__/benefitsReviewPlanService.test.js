...
// Mock Supabase and platformDb — wrapped in vi.hoisted so these are initialized
// before vi.mock's factory (which is itself hoisted above these consts) runs.
const { mockPlatformDb, mockSupabase } = vi.hoisted(() => ({
  mockPlatformDb: {
    from: vi.fn(() => ({
      select: vi.fn(() => ({
        eq: vi.fn(() => ({
