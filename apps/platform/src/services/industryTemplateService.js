...
/**
 * Resolve the industry plan template a project should default to, walking the
 * PM Template Hierarchy chain (project → programme/portfolio → PMO/Global).
 * Returns null when nothing is inherited yet — caller keeps the full catalog.
 */
export async function resolveEffectiveIndustryTemplate(projectId) {
  if (!projectId) return null
  const { data: proj, error: projErr } = await platformDb
    .from('projects')
    .select('account_id')
    .eq('id', projectId)
    .maybeSingle()
  throwIf(projErr, 'Failed to resolve project account')

  const node = await resolveEffectiveDocumentMaster(platformDb, 'project', projectId, 'industry_plan', {
    accountId: proj?.account_id || null,
  })
  if (!node?.domain_ref_id) return null
  return { templateId: node.domain_ref_id, tier: node.tier, scopeEntityId: node.scope_entity_id }
}
...
function planningFields(row = {}) {
  return {
    priority: row.priority || null,
    planned_hours: row.planned_hours === '' || row.planned_hours == null ? null : row.planned_hours,
    planned_cost: row.planned_cost === '' || row.planned_cost == null ? null : row.planned_cost,
    start_offset_days: row.start_offset_days === '' || row.start_offset_days == null ? null : row.start_offset_days,
    is_locked: Boolean(row.is_locked),
  }
}

function nestingFields(row = {}) {
  return {
...
function customFieldsPayload(row = {}) {
  const cf = row.custom_fields && typeof row.custom_fields === 'object' && !Array.isArray(row.custom_fields)
    ? row.custom_fields
    : {}
...
export async function replaceTemplateChildren(templateId, bundle) {
  ...
  for (const a of bundle.activities || []) {
    const { error } = await platformDb.from(ACTIVITY).insert({
      ...
      required_skills: Array.isArray(a.required_skills) ? a.required_skills : [],
      sort_order: a.sort_order ?? 0,
      ...planningFields(a),
      ...nestingFields(a),
      ...customFieldsPayload(a),
    })
