/**
 * Project delays + PMO delay templates (public schema, v353)
 */
...
export async function getDelaysByProject(projectId, filters = {}) {
  let q = platformDb
    .from('project_delays')
    .select(DELAY_SELECT)
...
export async function getAllDelays(orgId) {
...
export async function getDelayById(id) {
  const { data, error } = await platformDb.from('project_delays').select(DELAY_SELECT).eq('id', id).maybeSingle()
  if (error) throw error
  return data
}
...
export async function createDelay(payload) {
  ...
  const { data, error } = await platformDb.from('project_delays').insert(row).select(DELAY_SELECT).single()
  if (error) throw error
  return data
}

export async function updateDelay(id, patch, existingRow) {
  const clean = stripAutoImmutable(
    {
      ...patch,
  ...
  const { data, error } = await platformDb.from('project_delays').update(clean).eq('id', id).select(DELAY_SELECT).single()
  if (error) throw error
  return data
}
...
export async function getDraftDelays(projectId) {
  return getDelaysByProject(projectId, { draftsOnly: true })
}
...
export async function syncOverdueDelays(projectId) {
  const { data, error } = await platformDb.rpc('sync_overdue_delays', {
...
export async function getDelayTemplates(orgId, filters = {}) {
  ...
  if (filters.status) q = q.eq('status', filters.status)
  if (filters.category) q = q.eq('delay_category', filters.category)
  const { data, error } = await q
  if (error) throw error
  return data || []
}
...
export async function getDelayTemplate(id) {
  const { data, error } = await platformDb.from('delay_templates').select('*').eq('id', id).maybeSingle()
  if (error) throw error
  return data
}
...
export async function createDelayTemplate(payload) {
  ...
  const { data, error } = await platformDb.from('delay_templates').insert(row).select().single()
  if (error) throw error
  return data
}
...
/**
 * Pre-filled object for DelayForm (not persisted until user saves)
 */
export function copyTemplateToDelayObject(template, projectId, organisationId) {
