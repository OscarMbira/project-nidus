 ...
 * @param {{ applyFieldTemplateInheritance?: boolean, parentProgrammeId?: string|null, parentPortfolioId?: string|null, applyDefaults?: boolean }} [options]
...
export async function createProject(projectData, options = {}) {
  try {
    ...
    if (options.applyFieldTemplateInheritance === true && data?.id) {
      try {
        const accountId = projectData.account_id || options.accountId || (await getCurrentUserAccountId());
        if (accountId) {
          const parent = pickCreateParentLink({
            programmeId: options.parentProgrammeId || null,
            portfolioId: options.parentPortfolioId || null,
          });
          const result = await applyFieldTemplateInheritanceOnCreate(platformDb, {
            accountId,
            entityType: 'project',
            entityId: data.id,
            entityName: data.project_name,
            parentEntityType: parent.parentEntityType,
            parentEntityId: parent.parentEntityId,
            userId: projectData.created_by_user_id || projectData.owner_user_id || null,
...
export async function updateProject(projectId, projectData) {
  try {
    const { data: existing, error: fetchError } = await platformDb
      .from('projects')
      .select('*')
      .eq('id', projectId)
      .single();
    ...
    const governed = await tryGovernedLifecycleUpdate({
      db: platformDb,
      tableName: 'projects',
      recordId: projectId,
      currentRow: existing,
      updates: patch,
      notes: 'Project change submitted for authorisation',
    });

    if (governed.deferred) {
      return { success: true, data: governed.row, pendingAuthorisation: true };
    }
