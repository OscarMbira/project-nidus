/**
 * Practice Work Package Create Page
 */
...
export default function PracticeWorkPackageCreate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      const result = await createPracticeWorkPackage(projectId, formData)
      if (result.success) {
        navigate(`/simulator/practice-work-packages/${result.data.id}?projectId=${projectId}`)
      } else {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-6">
        ...
        <div>
          ...
          <textarea value={formData.objectives} onChange={(e) => setFormData({ ...formData, objectives: e.target.value })} rows={3} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-gray-700" />
        </div>
        <div className="flex justify-end gap-4">
          <button type="button" onClick={() => navigate(`/simulator/practice-work-packages?projectId=${projectId}`)} className="px-4 py-2 border rounded-lg">Cancel</button>
