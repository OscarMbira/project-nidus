...
export default function SimProjectTemplateCopyCreate() {
  ...
  const submit = async () => {
    if (!accountId || !templateId || !runId || !title.trim()) {
      setErr('Organisation, template, simulation run, and title are required')
      return
    }
    setSaving(true)
    setErr(null)
        ...
        >
          ...
          {runs.map((p) => (
            <option key={p.id} value={p.id}>
              {p.id.slice(0, 8)}… · {p.created_at ? new Date(p.created_at).toLocaleString() : ''}
            </option>
          ))}
        </select>
      </label>
