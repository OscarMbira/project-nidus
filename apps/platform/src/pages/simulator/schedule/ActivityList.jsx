...
import { simListActivities, simSaveActivity } from '../../../services/sim/simPlanningService'
...
export default function ActivityList() {
  ...
  const runBulk = async () => {
    ...
    for (const line of lines) {
      ...
      if (!name) continue
      const res = await simSaveActivity(projectId, { activity_code: code || null, name, status: 'not_started' }, user.id)
      if (res.success) ok += 1
    }
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canEdit && (
            <>
              ...
              <Link to={`/simulator/practice-projects/${projectId}/schedule/activities/new`} className="inline-flex items-center gap-1 rounded-lg bg-emerald-600 px-4 py-2 text-sm text-white">
      ...
      {loading ? (
            ...
            <tbody>
              {filtered.map((r, index) => (
                <tr key={r.id} className="border-t border-gray-200 dark:border-gray-700">
                  ...
                  <td className="p-3">
                    <Link to={`/simulator/practice-projects/${projectId}/schedule/activities/${r.id}`} className="text-blue-600 hover:underline dark:text-blue-400">
                      Open
                    </Link>
                  </td>
                </tr>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((r, index) => (
            <Link
              key={r.id}
              to={`/simulator/practice-projects/${projectId}/schedule/activities/${r.id}`}
