...
export default function ProjectITTOForm({
  open,
  onClose,
  onSaved,
  projectKey,
  ...
  useEffect(() => {
    ...
    if (initial) {
      ...
      setTailoringNotes(initial.tailoring_notes || '')
  ...
  const handleSave = async () => {
    ...
    if (!projectKey || !userId) {
      toast.error('Missing project or user context')
      return
    }
    setSaving(true)
    try {
      const common = {
        ...
        created_by: userId,
        template_id: templateId || initial?.template_id || null,
      }
      let row
      if (initial?.id) {
        ...
        toast.success(`Project ITTO updated — ID ${row.id}`)
      } else {
        row = await saveFn.create({
          ...common,
          ...saveFn.projectPayload(projectKey),
        })
        toast.success(`Project ITTO created — ID ${row.id}`)
      }
      onSaved?.(row)
      onClose()
    } catch (e) {
      ...
      <div className="w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-xl">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-900 z-10">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
            {initial?.id ? 'Edit project ITTO' : 'New project ITTO'}
          </h2>
        ...
        <div className="p-4">
          ...
          {formTab === 'audit' && (
            <div className="mt-4">
              {!initial?.id ? (
                <p className="text-sm text-gray-500 dark:text-gray-400">Audit details appear after this project ITTO is saved.</p>
              ) : (
                <AuditDetailsPanel description="Who created or changed this project ITTO, and how it is classified.">
                  <AuditCard title="Identity" description="How this ITTO record is labelled and tracked.">
                    <AuditField label="Name" value={name || initial.name} />
                    <AuditField label="Status" value={humanizeAuditToken(isDraft ? 'draft' : status)} />
                  </AuditCard>
                  <AuditCard title="Classification" description="Where this ITTO record sits.">
                  ...
                  <AuditCard title="Record history" description="When this ITTO record was created and last changed.">
          ...
          {step === 1 && (
            <div className="space-y-4">
              ...
              <label className="block">
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Tailoring notes</span>
                <textarea
          ...
          {step === 2 && (
            <div className="space-y-3">
              <p className="text-sm text-gray-600 dark:text-gray-400">Inputs</p>
              {inputs.map((row, idx) => (
