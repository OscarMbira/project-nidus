  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (data?.pm_template_node_id) {
        try {
          const map = await resolveTemplateProvenanceBatch(platformDb, [data.pm_template_node_id])
          if (!c) setProvenance(map.get(data.pm_template_node_id) || null)
        } catch {
          if (!c) setProvenance(null)
        }
      }
    })()
    return () => {
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap justify-between gap-4 mb-6">
        <div>
          ...
          {provenance && (
            <Link
              to={`/app/pmo/template-library/preview/${provenance.id}`}
