...
import { Table, TableBody, TableHeader, TableRow, TableHeaderCell, TableCell } from '@nidus/ui/Table'
...
export default function OPAList() {
  ...
  // v807 Gap 4: rows copied from a Global template link back to their source —
  // keyed by pm_template_node_id, batched (not N+1) since this is a list page.
  const [provenanceByNodeId, setProvenanceByNodeId] = useState(new Map())
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      setLoading(false)
      try {
        const map = await resolveTemplateProvenanceBatch(
          platformDb,
          (data || []).map((r) => r.pm_template_node_id),
        )
        if (!cancelled) setProvenanceByNodeId(map)
      } catch {
        if (!cancelled) setProvenanceByNodeId(new Map())
      }
    })()
    return () => {
      cancelled = true
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {displayRows.map((r, index) => (
            ...
            >
              ...
              {provenanceByNodeId.has(r.pm_template_node_id) && (
                <Link
                  to={`/app/pmo/template-library/preview/${provenanceByNodeId.get(r.pm_template_node_id).id}`}
                  className="text-xs text-emerald-600 hover:underline dark:text-emerald-400"
                  title="Copied from a Global template"
                >
                  from Global: {provenanceByNodeId.get(r.pm_template_node_id).name}
                </Link>
              )}
                ...
                <TableRow key={r.id}>
                  <TableCell className="font-medium text-gray-900 dark:text-white">
                    ...
                    {provenanceByNodeId.has(r.pm_template_node_id) && (
                      <Link
                        to={`/app/pmo/template-library/preview/${provenanceByNodeId.get(r.pm_template_node_id).id}`}
                        className="ml-2 text-xs font-normal text-emerald-600 hover:underline dark:text-emerald-400"
                        title="Copied from a Global template"
                      >
                        from Global
                      </Link>
