...
/**
 * PM Project Templates entry (v864).
 * Routes: /platform/templates/project[/:projectId]
 * Detail with project key: /platform/templates/project/:projectId/:nodeId (separate route)
 */
export default function ProjectTemplatesEntry() {
  ...
  /** @type {['pending'|'list'|'detail', Function]} */
  ...
  useEffect(() => {
    ...
    const dest = `/platform/templates/project/${encodeURIComponent(friendlyKey)}/${encodeURIComponent(pathSeg)}${qs ? `?${qs}` : ''}`
  ...
  if (!projectId && !pathSeg) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-12 text-center text-gray-700 dark:text-gray-300">
        <p>Select a project first to view and customise its templates.</p>
        <Link
