...
export default function TemplateOnHold() {
  ...
  const [viewMode, setViewMode] = useViewMode('platform-template-on-hold', 'list')

  const { handleSort, getSortDirectionForColumn, sortedData } = useSortableTable({
    defaultSort: { column: 'updated_at', direction: 'desc' },
    storageKey: 'nidus-template-on-hold-sort',
  })
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex flex-wrap justify-between items-center gap-2 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400">On-hold and draft copies.</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <ExportListMenu
            columns={EXPORT_COLS}
            data={exportRows}
            baseFilename="Template_Copies_On_hold"
      ...
      <div className="mb-4 flex flex-wrap gap-3 items-center">
        ...
        <ViewToggle value={viewMode} onChange={setViewMode} ariaLabel="Drafts layout" />
      </div>
      {loading ? (
