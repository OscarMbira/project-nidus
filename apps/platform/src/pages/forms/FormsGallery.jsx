  ...
  const templatesListHref = useMemo(
    () =>
      buildPmTemplatesListPath({
        pathname: mode === 'sim' ? '/simulator/pm/templates/project' : '/platform/templates/project',
        listVariant: 'project',
        projectKey: friendlyProjectKey || routeKey || projectId || '',
        searchParams: 'domainGroup=forms',
      }),
    [mode, friendlyProjectKey, routeKey, projectId],
  )

  useEffect(() => {
    const menuCode = mode === 'sim' ? 'sim_pm_project_templates_forms' : 'plat_pm_project_templates_forms'
    getMenuLabel(platformDb, menuCode, 'Forms').then(setBackLinkLabel)
  }, [mode])

  // Canonicalize /projects/<uuid>/forms → /projects/<project_code>/forms (rule 16.1 / v864).
  useEffect(() => {
    if (!projectId) {
  ...
  useEffect(() => {
    ...
    async function loadTemplates() {
      ...
      try {
        ...
        const rows = await listNearestFormTemplatesForProject(platformDb, {
          accountId,
          projectId,
          schema: 'public',
        })
