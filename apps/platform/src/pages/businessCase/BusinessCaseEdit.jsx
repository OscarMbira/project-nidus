...
import { FileText, Save, ArrowLeft, Loader2, Settings } from 'lucide-react'
import {
  getBusinessCaseById,
  canEditBusinessCase,
  updateBusinessCase,
...
import { resolveInitiationBasePath } from '@nidus/shared/utils/initiationRouteUtils'
import TierFieldCustomisationPanel from '@nidus/ui/TierFieldCustomisationPanel.jsx'
import InheritedBusinessCaseFields, {
  BUSINESS_CASE_CATEGORY,
} from '../../features/local-data-extensions/components/InheritedBusinessCaseFields'
...
export default function BusinessCaseEdit() {
  ...
  useEffect(() => {
    const projectId = businessCase?.project_id
    if (!projectId) {
      setAccountId(null)
      setProjectName('')
      return
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Tabs: Main form / Options / Benefits / Dis-benefits / Distribution / Settings */}
      <div className="flex flex-wrap gap-1 mb-4 border-b border-gray-200 dark:border-gray-700 pb-2">
        {[
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs font-medium rounded-lg inline-flex items-center gap-1 ${
              activeTab === tab.id
        ...
        {activeTab === 'settings' && (
          <div className="space-y-3">
            <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
              Business case field templates
            </h2>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Inherit fields from PMO / portfolio / programme defaults, then disable or add local fields
            ...
            {accountId && projectId ? (
              ...
              <p className="text-sm text-amber-700 dark:text-amber-300">
                Link this business case to a project to customise tier field templates.
              </p>
            )}
          </div>
        )}
