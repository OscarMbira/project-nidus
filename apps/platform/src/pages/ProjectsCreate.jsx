...
import {
  applyFieldTemplateInheritanceOnCreate,
  pickCreateParentLink,
} from '@nidus/shared/services/pmTemplateCreateInheritance.js'
...
export default function ProjectsCreate() {
  ...
  const [applyFieldTemplateInheritance, setApplyFieldTemplateInheritance] = useState(false)
  ...
  const handleSubmit = useCallback(async (e, isDraft = false) => {
    ...
    try {
      ...
      // Opt-in field-template inheritance (after parent links so programme preferred over portfolio)
      if (!isUpdate && applyFieldTemplateInheritance) {
        try {
          const accountId = project.account_id || (await getCurrentUserAccountId())
          if (accountId) {
            const parent = pickCreateParentLink({
              programmeId: selectedProgrammeId,
              portfolioId: selectedPortfolioId,
            })
            const result = await applyFieldTemplateInheritanceOnCreate(supabase, {
              accountId,
              entityType: 'project',
              entityId: project.id,
              entityName: project.project_name,
              parentEntityType: parent.parentEntityType,
              parentEntityId: parent.parentEntityId,
              userId: user.id,
              applyDefaults: true,
            })
            if (!result.ok) {
              console.warn('Project field-template inheritance skipped:', result.error)
            }
          }
        } catch (inhErr) {
          console.warn('Project field-template inheritance failed (non-fatal):', inhErr)
        }
      }
  ...
  }, [formData, validateForm, isPmoAdmin, navigate, fromMandate, selectedPortfolioId, selectedProgrammeId, applyFieldTemplateInheritance, createdProjectId])
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="space-y-3 pt-4">
          <label className="flex items-start gap-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 p-3 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
            <input
              type="checkbox"
              className="mt-0.5 rounded border-gray-300 dark:border-gray-600"
              checked={applyFieldTemplateInheritance}
              onChange={(e) => setApplyFieldTemplateInheritance(e.target.checked)}
            />
            <span>
              <span className="font-medium text-gray-900 dark:text-gray-100">Apply inherited field template</span>
              <span className="block text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                Opt-in: attach the programme / portfolio / PMO field template and copy default values onto this project. Off by default. Industry Plan structure (phases/activities) is unchanged.
              </span>
            </span>
          </label>
