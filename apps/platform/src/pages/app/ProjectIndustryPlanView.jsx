...
export default function ProjectIndustryPlanView({ isSim = false }) {
  ...
  useEffect(() => {
    const planSvc = getIndustryPlanService(isSim)
    const load = isSim ? planSvc.getPracticePlan(projectId) : planSvc.getProjectPlan(projectId)
    load
      .then(async (p) => {
        setPlan(p)
        if (p?.template_id) {
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                const planSvc = getIndustryPlanService(isSim)
                if (isSim) await planSvc.archivePracticePlan(plan.id)
                else await planSvc.archivePlan(plan.id)
