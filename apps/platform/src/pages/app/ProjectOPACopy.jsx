...
export default function ProjectOPACopy() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const { data } = await opaSvc.resolveEffectiveOpaMaster(projectId)
      if (cancelled || !data?.opaId) return
      setSourceOpaId(data.opaId)
      setInheritedFrom(data)
    })()
    return () => {
      cancelled = true
    }
      ...
      {step === 0 && (
        <div className="grid lg:grid-cols-2 gap-8">
          ...
          <div className="rounded-xl border border-gray-200 dark:border-gray-700 p-4 bg-gray-50 dark:bg-gray-900/40">
            ...
            {inheritedFrom && (
              <p className="mb-3 inline-block rounded-full bg-sky-100 dark:bg-sky-900/40 text-sky-700 dark:text-sky-300 text-xs font-medium px-3 py-1">
                Inherited from: {inheritedFrom.tier === 'pmo' ? 'PMO default' : inheritedFrom.tier}
              </p>
            )}
            {sourceOpa ? (
