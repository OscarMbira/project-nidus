...
export default function CustomFieldRenderer({
  ...
  /** When set, skip screen-map load and use these definitions (tier inheritance). */
  definitionsOverride = null,
  /** Optional Set/Array of definition ids to keep (intersection with loaded defs). */
  ...
  const skipScreenLoad = Array.isArray(definitionsOverride)
  const { definitions: screenDefs, loading: loadDef, error: errDef } = useCustomFields(
    platformDb,
    skipScreenLoad ? null : accountId,
    skipScreenLoad ? null : screenCode,
    { cacheScope }
  )
  ...
  const definitions = useMemo(() => {
    let defs = skipScreenLoad ? definitionsOverride || [] : screenDefs || []
    if (allowedFieldIds) {
      const allow = allowedFieldIds instanceof Set ? allowedFieldIds : new Set(allowedFieldIds)
      defs = defs.filter((d) => allow.has(d.id))
    }
    return defs
  }, [skipScreenLoad, definitionsOverride, screenDefs, allowedFieldIds])
  ...
  const loading = (skipScreenLoad ? false : loadDef) || loadVal
  const error = skipScreenLoad ? errVal : errDef || errVal
