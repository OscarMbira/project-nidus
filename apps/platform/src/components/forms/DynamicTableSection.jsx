export default function DynamicTableSection({ sectionKey, rows = [], onChange }) {
  ...
  return (
    <div className="space-y-2">
      {rows.map((row, idx) => (
        <div key={`${sectionKey}-${idx}`} className="rounded border border-gray-200 dark:border-gray-700 p-2">
          <textarea
            className="w-full rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-950 p-2 text-xs text-gray-900 dark:text-gray-100"
            rows={3}
            value={JSON.stringify(row, null, 2)}
            onChange={(e) => updateRow(idx, { raw: e.target.value })}
          />
          <button type="button" className="mt-2 text-xs text-red-600 dark:text-red-300" onClick={() => removeRow(idx)}>Remove row</button>
        </div>
      ))}
