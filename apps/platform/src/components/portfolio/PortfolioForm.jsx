    ...
    const init = async () => {
      if (portfolio) {
        ...
        setFormData({
          ...
          delivery_methodology_track: portfolio.delivery_methodology_track || '',
          tags: portfolio.tags || [],
        });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        delivery_methodology_track: formData.delivery_methodology_track || null,
        portfolio_goals:
      ...
      const saved = await savePortfolio(submitData, portfolio?.id, {
        applyFieldTemplateInheritance: !portfolio?.id && applyFieldTemplateInheritance,
      });
      try {
        onSave(saved);
      } catch (navError) {
              ...
              <div className="md:col-span-2 space-y-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <select
                      name="delivery_methodology_track"
                      value={formData.delivery_methodology_track}
                      onChange={handleChange}
                      className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    >
                      <option value="">Inherit (org default / ancestor)</option>
                    ...
                    <p className="mt-1 text-xs text-gray-500 dark:text-gray-400 flex items-start gap-1">
                      ...
                      <span>
                        Optional. When set, Template Library and methodology focus use this track for this
                        portfolio (and descendants that do not set their own).
                      </span>
                    </p>
                  </div>
                </div>
            ...
            <label className="flex items-start gap-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/60 p-3 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
              ...
              <span>
                ...
                <span className="block text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                  Opt-in: attach the parent portfolio / PMO field template and copy default values onto this portfolio. Off by default.
                </span>
              </span>
            </label>
          )}
