...
export default function StandardForm({ standardData = {}, onChange, onCancel, onSubmit, isEditing = false }) {
  ...
  useEffect(() => {
    if (formTab !== 'audit' || !standardData?.id) return
    let cancelled = false
    ;(async () => {
      const labels = await resolveAuditUserLabels(platformDb, [standardData.created_by])
      if (!cancelled) setAuditUserLabels(labels || {})
    })()
    return () => { cancelled = true }
  }, [formTab, standardData?.id, standardData?.created_by])
  ...
  const standardTypes = [
    ...
    { value: 'standards_based', label: 'Process Guide' },
        ...
        !standardData?.id ? (
          ...
          <AuditDetailsPanel description="How this standard is labelled and classified, and when it was created.">
            <AuditCard title="Identity" description="How this standard is labelled and tracked.">
              ...
              <AuditField label="Code" value={standardData.standard_code} />
              <AuditField label="Type" value={humanizeAuditToken(standardData.standard_type)} />
            </AuditCard>
            <AuditCard title="Classification" description="Compliance level for this standard.">
            ...
            <AuditCard title="Record history" description="When this standard was created.">
              <AuditField label="Created by" value={standardData.created_by ? auditUserLabels[standardData.created_by] || null : null} />
