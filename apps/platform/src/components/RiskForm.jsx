...
function buildRiskFormSnapshot(data) {
  return JSON.stringify({
    risk_title: data.risk_title || '',
    risk_code: data.risk_code || '',
    risk_description: data.risk_description || '',
    risk_category: data.risk_category || '',
    risk_type: data.risk_type || 'threat',
    probability: data.probability ?? 3,
    impact: data.impact ?? 3,
    response_strategy: data.response_strategy || '',
    response_strategy_description: data.response_strategy_description || '',
    risk_owner_user_id: data.risk_owner_user_id || '',
    target_mitigation_date: data.target_mitigation_date || '',
...
function riskToFormData(risk, linkedTaskId, linkedWorkPackageId) {
  if (!risk) {
    return {
      ...EMPTY_RISK_FORM,
...
export default function RiskForm({ risk, projectId, onSave, onCancel, onHoldComplete, linkedTaskId, linkedWorkPackageId }) {
  ...
  useEffect(() => {
    const next = riskToFormData(risk, linkedTaskId, linkedWorkPackageId)
    setFormData(next)
    setBaselineSnapshot(buildRiskFormSnapshot(next))
    fetchTeamMembers()
    fetchLinkedItems()
