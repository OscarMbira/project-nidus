/**
 * Template Library — Platform (public.template_library)
 */
...
export async function getTemplatesForAccount(accountId, { manage = false, search = '', typeCode = '', categoryId = '' } = {}) {
  ...
  let q = platformDb
    ...
    .select(
      `
      *,
      category:${CAT}(id, category_code, category_name)
    `
    )
...
export async function createTemplate(payload) {
  ...
  const { data, error } = await platformDb.from(TLIB).insert(row).select().single()
  return { data: mapTemplateRow(data), error }
}
...
export async function updateTemplate(id, patch) {
  ...
  const { data, error } = await platformDb.from(TLIB).update(row).eq('id', id).select().single()
  return { data: mapTemplateRow(data), error }
}
...
export async function upsertTemplateCategory({ id, category_code, category_name, description, sort_order = 0 }) {
  ...
  if (id) {
    const { data, error } = await platformDb.from(CAT).update(row).eq('id', id).select().single()
    return { data, error }
  }
  const { data, error } = await platformDb.from(CAT).insert(row).select().single()
  return { data, error }
}

export async function deleteTemplateCategory(id) {
  const { error } = await platformDb.from(CAT).update({ is_active: false, updated_at: new Date().toISOString() }).eq('id', id)
  return { error }
}
...
/**
 * Bulk import master templates from parsed CSV rows.
 * Expected columns: title, template_type_code, category_code (optional), description, status (optional draft|published)
 */
export async function bulkImportTemplatesFromRows(accountId, rows, { onProgress } = {}) {
  ...
  const { data: cats } = await platformDb.from(CAT).select('id, category_code')
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const { error } = await platformDb.from(TLIB).insert({
