...
export const LEGACY_CATEGORY_SHELL_TARGETS = {
  ...
  'plat_sec_pmbok':               null,
  'plat_grp_pmbok_forms':         'pmo-cat-pmbok',
  'plat_sec_agile':               null,
...
function inferPlatformCategoryId(node = {}) {
  ...
  if (/pmo-cat-pmbok|process.group|process_group|\/forms\?group=/.test(signal)) {
    return 'pmo-cat-pmbok'
  }
  ...
  if (
    /\/platform\/teams|\/pm\/teams|manager assignment|appointment tracker|people.resource|people & resources|team capacity|resource directory|send invitation|assign roles to project|invitation-tracker|send-role-invites/.test(
      signal
    )
  ) {
  ...
  if (
    /\/platform\/admin\/(invitation|send-role|assign-roles|email-settings|email-sender|invitation-settings)/.test(path)
  ) {
...
function inferSimPmoCategoryId(node = {}) {
  ...
  if (/sim_pmo_cat_pmbok|process group/.test(signal)) return 'sim_pmo_cat_pmbok'
...
const PM_TRACK_AND_GROUP_CODE_TARGETS = {
  ...
  plat_grp_pm_process_groups: 'pmo-cat-pmbok',
  plat_grp_pm_itto: 'pmo-cat-pmbok',
  ...
  plat_grp_pmbok_process: 'pmo-cat-pmbok',
  plat_grp_itto: 'pmo-cat-pmbok',
...
function inferPmPlatformCategoryId(node = {}) {
  ...
  if (/^plat_pm_p_/.test(code)) return 'pmo-cat-pmbok'
  ...
  if (/plat_grp_pm_teams|all teams|my team|resource directory|skill matrix|capacity|leave calendar/.test(signal)) {
  ...
  if (/itto framework|itto template|process group forms|\/itto\//.test(signal)) return 'pmo-cat-pmbok'
...
function inferSimPmCategoryId(node = {}) {
  ...
  if (/sim_pm_cat_pmbok|process group/.test(signal)) return 'sim_pm_cat_pmbok'
