...
const MENU_CACHE_KEY_PREFIX = 'nidus_menu_v2_'
...
function getCacheKeys(userId) {
  return [`${MENU_CACHE_KEY_PREFIX}${userId}`, `${LEGACY_MENU_CACHE_KEY_PREFIX}${userId}`]
}
...
function applyRoleSidebarRevamp(menuItems = []) {
  ...
  const hasPMOContext = baseline.some((n) => {
    const s = signal(n)
    return s.includes('/pmo/') || s.includes('pmo') || s.includes('oversight') || s.includes('programme') || s.includes('portfolio')
  })

  if (!hasPMOContext) return baseline
  ...
  const categoryDefs = [
    ...
    { id: 'pmo-cat-portfolio', label: 'Portfolio & Programme Management', order: 2 },
    { id: 'pmo-cat-project-oversight', label: 'Project Oversight', order: 3 },
    { id: 'pmo-cat-delivery-controls', label: 'Delivery Controls', order: 4 },
    { id: 'pmo-cat-financial-commercial', label: 'Financial & Commercial Management', order: 5 },
    { id: 'pmo-cat-risk-issues-quality', label: 'Risk, Issues & Quality', order: 6 },
    { id: 'pmo-cat-governance-standards', label: 'Governance & Standards', order: 7 },
    { id: 'pmo-cat-reporting-intelligence', label: 'Reporting & Intelligence', order: 8 },
    { id: 'pmo-cat-workflows-approvals', label: 'Workflows & Approvals', order: 9 },
    { id: 'pmo-cat-people-stakeholders', label: 'People & Stakeholders', order: 10 },
    { id: 'pmo-cat-knowledge-assets', label: 'Knowledge & Assets', order: 11 },
    { id: 'pmo-cat-audit-compliance', label: 'Audit Trail & Compliance', order: 12 },
    { id: 'pmo-cat-admin', label: 'PMO Administration', order: 13 },
    { id: 'pmo-cat-help', label: 'Help', order: 14 },
    { id: 'pmo-cat-support', label: 'Support', order: 15 },
  ]

  const categoryFallbacks = {
    'pmo-cat-exec': { label: 'PMO Dashboard', path: '/platform/dashboard' },
    'pmo-cat-portfolio': { label: 'Portfolio View', path: '/platform/dashboard' },
    ...
    'pmo-cat-knowledge-assets': { label: 'Template Library', path: '/platform/projects/templates' },
  ...
  const matchCategory = (item) => {
    ...
    if (/portfolio|programme|program|benefits pipeline|strategic alignment|investment|roadmap/.test(s)) return 'pmo-cat-portfolio'
    ...
    if (/scope|schedule|milestone|dependency|baseline|delivery performance|delay/.test(s)) return 'pmo-cat-delivery-controls'
    if (/budget|cost|forecast|procurement|vendor|commercial|expense|financial/.test(s)) return 'pmo-cat-financial-commercial'
    if (/risk register|enterprise risk|issue register|quality assurance|audit findings|capa|quality register/.test(s)) return 'pmo-cat-risk-issues-quality'
    ...
    if (/resource|stakeholder|role|accountability|communications overview|team capacity|engagement|manager assignment/.test(s)) return 'pmo-cat-people-stakeholders'
    ...
    if (/admin|organisation settings|branding|menu configuration|approval rules|master data|configuration|role menu access|subscription|pwa settings|\/platform\/settings/.test(s)) return 'pmo-cat-admin'
