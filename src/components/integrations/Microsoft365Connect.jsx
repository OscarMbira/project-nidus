...
import { initiateMicrosoft365OAuth, completeMicrosoft365OAuth, getMicrosoft365Connection, refreshMicrosoft365Token } from '../../services/microsoft365Service'
import { supabase } from '../../services/supabaseClient'
import { CheckCircle, XCircle, RefreshCw, Plug, Save, X } from 'lucide-react'

export default function Microsoft365Connect({ onConnected }) {
  ...
  const fetchConnection = async () => {
    try {
      ...
      const result = await getMicrosoft365Connection()
      if (result.success && result.data) {
        setConnection(result.data)
        if (result.data.connected_services) {
          setServices({
            teams: result.data.connected_services.includes('teams'),
            outlook: result.data.connected_services.includes('outlook'),
  ...
  const handleConnect = async () => {
    try {
      ...
      const result = await initiateMicrosoft365OAuth(redirectUri)

      if (result.success && result.authUrl) {
        // Store callback info in session storage
        sessionStorage.setItem('m365_oauth_redirect', window.location.pathname)
      ...
      alert('Failed to connect to Microsoft 365')
    } finally {
      setConnecting(false)
    }
  }
  ...
  const handleRefreshToken = async () => {
    try {
      setConnecting(true)
      const result = await refreshMicrosoft365Token()
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Connect with Microsoft Teams, Outlook, and Calendar
            </p>
          </div>
        </div>
      </div>
      ...
      {!connection ? (
        <div className="text-center py-8">
          <p className="text-gray-600 dark:text-gray-400 mb-4">
            Connect your Microsoft 365 account to sync calendars and send notifications
          </p>
          <button
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-green-700 dark:text-green-300 mt-1">
                    Tenant ID: {connection.tenant_id || 'N/A'}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-3">
              <label className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <input
                  ...
                  onChange={(e) => setServices({ ...services, teams: e.target.checked })}
                  className="ml-3"
                />
              </label>
              ...
              <label className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <input
                  type="checkbox"
                  checked={services.outlook}
                  onChange={(e) => setServices({ ...services, outlook: e.target.checked })}
                  className="ml-3"
                />
              </label>
              ...
              <label className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700">
                <div>
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400">
                    Sync tasks and milestones to Outlook Calendar
                  </p>
                </div>
                <input
                  type="checkbox"
