...
export default function SimulatorPMOLayout({ children }) {
  ...
  return (
    <div className={`h-screen flex flex-col overflow-hidden ${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-100'}`}>
      {/* Top Navigation */}
      <nav className={`fixed top-0 left-0 right-0 z-50 ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-b`}>
        <div className="px-4 h-16 flex items-center justify-between">
          <div className="flex items-center">
            ...
            <Link to="/simulator/pmo/dashboard" className="ml-4 flex items-center">
              <span className="text-2xl font-bold bg-gradient-to-r from-indigo-500 to-purple-600 bg-clip-text text-transparent">
                Practice PMO Dashboard
              </span>
            </Link>
          </div>
