/**
 * Work Package Reporting Arrangements Section Component
 * Displays and manages Work Package reporting arrangements
 */

import { useState, useEffect } from 'react'
import { Plus, FileText } from 'lucide-react'
import { getReportingArrangements, deleteReportingArrangement } from '../../services/wpReportingArrangementsService'
import WPReportingArrangementCard from './WPReportingArrangementCard'
import WPReportingArrangementForm from './WPReportingArrangementForm'

export default function WPReportingArrangementsSection({ wpId, mode = 'view', projectId }) {
  ...
  const loadArrangements = async () => {
    try {
      setLoading(true)
      const result = await getReportingArrangements(wpId)
      if (result.success) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            How progress and status will be reported
          </p>
        </div>
        {mode !== 'view' && (
          <button
      ...
      {arrangements.length === 0 ? (
        <div className="bg-gray-50 dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-12 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">
            Add reporting arrangements for this work package
          </p>
          {mode !== 'view' && (
            <button
              onClick={handleAdd}
