/**
 * Stage Plan Form Component
 * Wizard-style form for creating/editing Stage Plans
 */

import { useState, useEffect } from 'react'
import { FileText, Calendar, DollarSign, Users, Package, Shield, CheckCircle, ArrowRight, ArrowLeft, X, Save } from 'lucide-react'
import { useThemeContext } from '../../context/ThemeContext'
import { createStagePlan, updateStagePlan, getStagePlanById, validateCompleteness } from '../../services/stagePlanService'
...
const FORM_STEPS = [
  { id: 'overview', label: 'Overview', icon: FileText, description: 'Stage plan overview' },
  { id: 'schedule', label: 'Schedule', icon: Calendar, description: 'Dates, milestones' },
  { id: 'budget', label: 'Budget', icon: DollarSign, description: 'Stage budget' },
...
export default function StagePlanForm({
  projectId,
  projectPlanId,
  stageNumber,
  planId = null,
  ...
  const validateStep = () => {
    ...
    if (step === 'overview') {
      if (!formData.stage_name || formData.stage_name.length < 3) {
        newErrors.stage_name = 'Stage name must be at least 3 characters'
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (mode === 'create') {
        result = await createStagePlan(projectId, stageNumber, formData)
      } else {
        result = await updateStagePlan(planId, formData)
      }
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-50 dark:bg-gray-800 p-6 rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                <p><strong>Stage:</strong> {formData.stage_name || 'Not set'}</p>
                ...
                <p><strong>Budget:</strong> {formData.stage_budget ? `$${formData.stage_budget}` : 'Not set'}</p>
        ...
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              {mode === 'create' ? 'Create Stage Plan' : mode === 'edit' ? 'Edit Stage Plan' : 'View Stage Plan'}
            </h1>
