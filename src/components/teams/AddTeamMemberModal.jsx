...
export default function AddTeamMemberModal({ isOpen, onClose, teamId, projectId, onSuccess }) {
  ...
  useEffect(() => {
    if (!isOpen || !teamId || !projectId) return
    let cancelled = false
    ;(async () => {
      setLoading(true)
      const [a, r] = await Promise.all([
        getAssignableProjectMembers(projectId, teamId),
        getTeamFunctionalRoles(teamId),
      ])
      if (cancelled) return
      ...
      if (r.success) {
        ...
        setPresetRole(dev?.id ? dev.role_label : list[0]?.role_label || '')
      }
      setLoading(false)
    })()
    return () => {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const res = await getUserTotalTeamAllocation(selectedUserId)
      if (res.success && res.total >= 100) {
        setAllocWarning('This user is already at 100% allocation across other teams.')
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      const res = await addTeamMember(teamId, selectedUserId, resolvedRole, pct)
      if (res.success) {
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-lg shadow-xl max-w-lg w-full p-6 border border-gray-200 dark:border-gray-700 my-8">
        ...
        {loading ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Project member</label>
              <select
                required
              ...
              >
                ...
                {filtered.map((row) => {
                  const u = row.users
                  const pr = row.role
                  const label = `${u?.full_name || u?.email || 'User'} — ${pr?.role_display_name || pr?.role_name || 'Member'}`
                  return (
