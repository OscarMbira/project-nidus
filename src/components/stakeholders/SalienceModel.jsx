/**
 * SalienceModel – Mitchell et al. salience: list stakeholders by salience class.
 * Uses latest analysis per stakeholder; shows 7+1 classes (definitive, dominant, dangerous, etc.).
 */
...
const SALIENCE_LABELS = {
  definitive: 'Definitive (P+L+U)',
  dominant: 'Dominant (P+L)',
  dangerous: 'Dangerous (P+U)',
  dependent: 'Dependent (L+U)',
  demanding: 'Demanding (U)',
  discretionary: 'Discretionary (L)',
  dormant: 'Dormant (P)',
  latent: 'Latent (none)',
}
...
export default function SalienceModel({ projectId }) {
  ...
  // Latest analysis per stakeholder; include salience_class (from record or compute from P/L/U)
  ...
  if (!projectId) {
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 p-6 text-center text-gray-500 dark:text-gray-400">
        Select a project to view the salience model.
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <ExportListMenu columns={EXPORT_COLUMNS} data={exportData} baseFilename="Stakeholder-Salience" disabled={!exportData.length} />
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {SALIENCE_CLASSES.map((cls) => (
          <div
            key={cls}
            className={`rounded-lg border p-4 ${SALIENCE_COLORS[cls] || SALIENCE_COLORS.latent}`}
          >
            ...
            <ul className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
              {(byClass[cls] || []).length === 0 ? (
                ...
                (byClass[cls] || []).map((rec) => (
                  <li key={rec.id}>
                    {rec.stakeholder?.stakeholder_name || rec.stakeholder_id || '—'}
                  </li>
                ))
              )}
            </ul>
