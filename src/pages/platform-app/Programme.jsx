...
export default function Programme() {
  ...
  // Initial load: fetch programme list only (avoid extra auth/account lookups)
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    getProgrammesForList({})
      .then((data) => {
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading programmes...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Programme</h1>
            </div>
        ...
        {loading ? (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-12 text-center">
            <Layers className="h-16 w-16 text-gray-500 dark:text-gray-600 mx-auto mb-4" />
            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-300 mb-2">No Programmes Found</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredProgrammes.map(programme => (
              <div
                ...
                className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 hover:border-blue-500 transition-colors cursor-pointer"
              >
                <div className="flex items-start justify-between gap-2 mb-4">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-1 truncate">{programme.programme_name}</h3>
                    {programme.programme_code && (
                      <p className="text-sm text-gray-600 dark:text-gray-400">{programme.programme_code}</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                      title="View"
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-amber-500 dark:hover:text-amber-400 hover:bg-gray-100 dark:hover:bg-gray-700"
                      title="Edit"
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50"
                      title="Delete"
                ...
                {programme.programme_description && (
                  <p className="text-gray-600 dark:text-gray-400 mb-4 line-clamp-2 text-sm">{programme.programme_description}</p>
                )}
                <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-500">
                  ...
                  {programme.programme_owner && (
                    <span className="text-gray-600 dark:text-gray-400 truncate ml-2">Owner: {programme.programme_owner.full_name || programme.programme_owner.email}</span>
                  )}
                </div>
              </div>
            ))}
        ...
        {filteredProgrammes.length > 0 && (
          <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">Total Programmes</p>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
              <div className="flex items-center gap-3 mb-2">
                <GitBranch className="h-6 w-6 text-blue-400" />
                <span className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.active}</span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">Active Programmes</p>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
              <div className="flex items-center gap-3 mb-2">
                <Target className="h-6 w-6 text-green-400" />
                <span className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.planning}</span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">In Planning</p>
            </div>
            <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
              <div className="flex items-center gap-3 mb-2">
                <Calendar className="h-6 w-6 text-yellow-400" />
                <span className="text-2xl font-bold text-gray-900 dark:text-gray-100">{stats.onHold}</span>
              </div>
