...
function utilPct(approved, spent) {
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-2 bg-gray-700 rounded-full overflow-hidden">
        <div className={`h-full ${colour}`} style={{ width: `${Math.min(pct, 100)}%` }} />
      </div>
...
const EMPTY_FORM = {
  portfolio_id: '', budget_name: '', budget_type: '', budget_currency: 'USD',
  approved_amount: '', actual_spent: '', remaining_amount: '', budget_status: 'draft',
}

export default function PortfolioFinancial() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [data, pf] = await Promise.all([getAllPortfolioBudgets(filters), getPortfolioList()])
      setRows(data); setPortfolios(pf)
    } catch (err) { setError(err.message) }
    finally { setLoading(false) }
  }, [filterPortfolio, filterType, filterStatus, filterCurrency])
  ...
  const filtered = rows.filter(r => {
    ...
    return (
      r.budget_name?.toLowerCase().includes(q) ||
  ...
  function openAdd() {
    setEditRow(null); setForm(EMPTY_FORM); setFormError(null); setShowModal(true)
  }
  ...
  function openEdit(r) {
    ...
    setForm({
      ...
      budget_status:   r.budget_status  || 'draft',
    })
    setFormError(null); setShowModal(true)
  }

  async function handleSave() {
    if (!form.portfolio_id || !form.budget_name) { setFormError('Portfolio and budget name are required.'); return }
    setSaving(true); setFormError(null)
    try {
      const payload = {
        ...form,
        approved_amount:  form.approved_amount  !== '' ? Number(form.approved_amount)  : null,
        actual_spent:     form.actual_spent     !== '' ? Number(form.actual_spent)     : null,
        remaining_amount: form.remaining_amount !== '' ? Number(form.remaining_amount) : null,
      }
      await savePortfolioBudget(payload, editRow?.id || null)
      setShowModal(false)
      flash(editRow ? 'Budget updated.' : 'Budget added.')
      load()
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await deletePortfolioBudget(deleteTarget.id)
  ...
  const exportColumns = [
    ...
    { key: 'budget_currency', header: 'Currency' },
  ]
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Portfolio Financial</h1>
          <p className="text-gray-400 text-sm mt-1">Budget & financial overview across all portfolios</p>
        </div>
        <div className="flex items-center gap-2">
          <ExportListMenu data={filtered} columns={exportColumns} baseFilename="portfolio-financial" />
      ...
      <div className="flex flex-wrap gap-3">
        ...
        {[
          { val: filterPortfolio, set: setFilterPortfolio, placeholder: 'All Portfolios', opts: portfolios.map(p => ({ v: p.id, l: `${p.portfolio_code} — ${p.portfolio_name}` })) },
          { val: filterType,      set: setFilterType,      placeholder: 'All Types',      opts: BUDGET_TYPES.map(t => ({ v: t, l: t })) },
          { val: filterStatus,    set: setFilterStatus,    placeholder: 'All Statuses',   opts: BUDGET_STATUSES.map(s => ({ v: s, l: s })) },
          { val: filterCurrency,  set: setFilterCurrency,  placeholder: 'All Currencies', opts: CURRENCIES.map(c => ({ v: c, l: c })) },
        ].map((f, i) => (
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map((r, i) => {
                  const pct = utilPct(r.approved_amount, r.actual_spent)
                  return (
                    ...
                    <TableRowNumberCell number={getDisplayRowNumber(index)} />
                      ...
                      <td className="px-4 py-3 w-32"><UtilBar pct={pct} /></td>
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-lg p-6 space-y-4 max-h-screen overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={form.budget_name} onChange={e => setForm(f => ({ ...f, budget_name: e.target.value }))}
                  placeholder="e.g. FY2026 Capital Budget"
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-gray-400 text-sm">
              Delete <strong className="text-white">{deleteTarget.budget_name}</strong>? This action cannot be undone.
            </p>
