...
function modelBadge(m) {
  ...
  return m ? (
    <span className={`${map[m] || 'bg-gray-600 text-gray-200'} text-xs px-2 py-0.5 rounded-full`}>{m}</span>
  ) : <span className="text-gray-500 text-xs">—</span>
}

const EMPTY_EDIT = {
  governance_model: '', review_frequency: '', last_review_date: '',
  next_review_date: '', decision_authority: '', governance_notes: '',
}
...
export default function PortfolioGovernance() {
  ...
  // Edit modal
  const [editRow, setEditRow]     = useState(null)  // governance record or null (create)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [data, pf] = await Promise.all([getAllPortfolioGovernance(filters), getPortfolioList()])
  ...
  function openEdit(r) {
    setEditRow(r)
    setEditPortfolioId(r.portfolio_id)
    setForm({
  ...
  async function handleSave() {
    ...
    try {
      await savePortfolioGovernance(editPortfolioId, form)
      setEditRow(null); setEditPortfolioId(null)
  ...
  // Portfolios without a governance record (to allow creating one)
  const portfoliosWithGov = new Set(rows.map(r => r.portfolio_id))
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Portfolio Governance</h1>
          <p className="text-gray-400 text-sm mt-1">Governance settings for each portfolio</p>
        </div>
        <div className="flex items-center gap-2">
          <ExportListMenu data={filtered} columns={exportColumns} baseFilename="portfolio-governance" />
        </div>
      </div>
      ...
      {!loading && portfoliosWithoutGov.length > 0 && (
        <div className="bg-yellow-900/20 border border-yellow-700/50 rounded-lg p-4">
          <p className="text-yellow-300 text-sm font-medium mb-2">Portfolios without a governance record:</p>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search portfolio or model…"
        ...
        <select value={filterModel} onChange={e => setFilterModel(e.target.value)}
          ...
          {GOVERNANCE_MODELS.map(m => <option key={m} value={m}>{m}</option>)}
        </select>
              ...
              <tbody>
                {filtered.map((r, i) => (
                  <tr key={r.id} className={`border-b border-gray-700/50 hover:bg-gray-700/30 ${i % 2 ? 'bg-gray-700/10' : ''}`}>
                    ...
                    <td className="px-4 py-3 text-gray-300">{r.decision_authority || '—'}</td>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-lg p-6 space-y-4 max-h-screen overflow-y-auto">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setEditPortfolioId(null); setEditRow(null) }} className="text-gray-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {!editRow && (
              <p className="text-gray-400 text-sm">
                Portfolio: <strong className="text-white">{portfolios.find(p => p.id === editPortfolioId)?.portfolio_name || editPortfolioId}</strong>
              </p>
            )}
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select value={form.governance_model} onChange={e => setForm(f => ({ ...f, governance_model: e.target.value }))}
                ...
                <div>
                  ...
                  <select value={form.review_frequency} onChange={e => setForm(f => ({ ...f, review_frequency: e.target.value }))}
                    ...
                    <option value="">—</option>
                    {REVIEW_FREQUENCIES.map(rf => <option key={rf} value={rf}>{rf}</option>)}
                  </select>
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-gray-400 text-sm mb-1">Last Review Date</label>
                  <input type="date" value={form.last_review_date} onChange={e => setForm(f => ({ ...f, last_review_date: e.target.value }))}
                    className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm focus:outline-none" />
                </div>
                <div>
                  <label className="block text-gray-400 text-sm mb-1">Next Review Date</label>
              ...
              <div>
                ...
                <input value={form.decision_authority} onChange={e => setForm(f => ({ ...f, decision_authority: e.target.value }))}
                  placeholder="e.g. Portfolio Board, PMO Director"
              ...
              <div>
                <label className="block text-gray-400 text-sm mb-1">Governance Notes</label>
                <textarea value={form.governance_notes} onChange={e => setForm(f => ({ ...f, governance_notes: e.target.value }))}
                  rows={3} placeholder="Additional governance notes…"
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button onClick={() => { setEditPortfolioId(null); setEditRow(null) }}
