...
import {
  getPracticeTestCaseById,
  updatePracticeTestCase,
} from '../../../services/sim/practiceTestCaseService'
...
export default function SimTestCaseDetail() {
  const { caseId } = useParams()
  return (
    <SimTestingPageShell title="Test case" subtitle="View and edit steps and metadata.">
...
function Detail({ caseId, projectId }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const data = await getPracticeTestCaseById(key)
      if (!data) {
        setTc(null)
        setNotFound(true)
      } else {
  ...
  useEffect(() => {
    if (!projectId) return
    ;(async () => {
      const s = await getPracticeTestSuites(projectId)
      setSuites(s)
    })()
  }, [projectId])
    ...
    return (
      <div className="rounded-xl border border-gray-800 bg-gray-900/40 p-6 space-y-3 text-gray-300 text-sm">
        ...
        <p className="text-gray-400">
          This link may be out of date. Open the case from{' '}
          <Link to={`${SIM_TESTING_BASE}/cases`} className="text-emerald-400 hover:underline">
            Test cases
          </Link>{' '}
          or use the ref in the URL (<code className="text-gray-300">TC-…</code>).
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap justify-between gap-2">
        <Link to={`${SIM_TESTING_BASE}/cases`} className="text-sm text-emerald-400 hover:underline">
          ← All cases
        </Link>
      ...
      {editing ? (
        <TestCaseForm
          ...
          onSave={async (payload) => {
            ...
            await updatePracticeTestCase(tc.id, { ...rest, steps })
            setEditing(false)
            load()
          }}
        />
