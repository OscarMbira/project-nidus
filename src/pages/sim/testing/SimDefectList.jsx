...
export default function SimDefectList() {
  return (
    <SimTestingPageShell title="Defects" subtitle="Practice project defect register.">
...
function Body({ projectId }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const data = await getPracticeDefects(projectId, {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2 items-center justify-between">
        <button
          type="button"
          onClick={() => navigate(`${SIM_TESTING_BASE}/defects/new`)}
          className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm"
        >
          <Plus className="h-4 w-4" />
          New defect
        </button>
        <ExportListMenu columns={EXPORT_COLS} data={rows} baseFilename="practice_defects" disabled={!rows.length} />
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-gray-800 overflow-hidden">
          ...
          {rows.length === 0 && <p className="p-4 text-gray-500 text-sm">No defects for this practice project.</p>}
        </div>
      )}
    </div>
  )
