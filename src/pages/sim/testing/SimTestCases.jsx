...
export default function SimTestCases() {
  return (
    <SimTestingPageShell title="Test cases" subtitle="All cases for the selected practice project.">
...
function CasesBody({ projectId }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const data = await getPracticeTestCases(projectId, { suite_id: suiteFilter || undefined })
      setRows(data)
    } finally {
      setLoading(false)
    }
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap justify-between gap-2">
        <Link
          to={
            suiteFilter
              ? `${SIM_TESTING_BASE}/cases/new?suite=${encodeURIComponent(suiteFilter)}`
              : `${SIM_TESTING_BASE}/cases/new`
          }
        ...
        <ExportListMenu columns={EXPORT_COLS} data={rows} baseFilename="practice_test_cases" disabled={!rows.length} />
      </div>
      {loading ? (
