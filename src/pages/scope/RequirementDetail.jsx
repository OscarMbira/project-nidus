...
export default function RequirementDetail() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    const res = await getRequirement(projectId, reqId)
  ...
  const save = async (asDraft) => {
    ...
    try {
      ...
      if (isNew && res.data?.id) {
        navigate(`/platform/projects/${projectId}/scope/requirements/${res.data.id}`, { replace: true })
      }
    } catch (e) {
  ...
  const del = async () => {
    ...
    const { data: { user } } = await platformDb.auth.getUser()
    if (!user) return
    const res = await softDeleteRequirement(reqId, projectId, user.id)
    if (res.success) navigate(`/platform/projects/${projectId}/scope/requirements`)
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 dark:bg-gray-950">
      <nav className="mb-4 text-sm text-gray-500 dark:text-gray-400">
        ...
        <Link to={`/platform/projects/${projectId}/scope/requirements`} className="hover:underline">
          Requirements
        </Link>
        <span className="mx-2">/</span>
