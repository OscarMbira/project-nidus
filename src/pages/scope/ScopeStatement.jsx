...
import { getScopeStatementByProject, saveScopeStatement } from '../../services/scopeStatementService'
...
export default function ScopeStatement() {
  ...
  const load = useCallback(async () => {
    if (!projectId) return
    setLoading(true)
    const res = await getScopeStatementByProject(projectId)
  ...
  const save = async (asDraft) => {
    ...
    try {
      ...
      const res = await saveScopeStatement(
        projectId,
        {
    ...
    <div className="mx-auto max-w-4xl px-4 py-8 dark:bg-gray-950">
      ...
      <div className="mb-6 flex flex-wrap justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Define scope (PMBOK 5.4).</p>
        </div>
        <ExportRecordButtons
