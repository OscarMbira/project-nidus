...
import {
  getAllSimPortfolioReports,
  getSimPortfolioList,
  saveSimPortfolioReport,
...
export default function SimPortfolioReports() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [data, pf] = await Promise.all([
        getAllSimPortfolioReports(filters),
        getSimPortfolioList(),
      ])
      setRows(data)
      setPortfolios(pf)
  ...
  function openEdit(r) {
    setEditRow(r)
    setForm({
      practice_portfolio_id: r.practice_portfolio_id || '',
  ...
  async function handleSave() {
    if (!form.practice_portfolio_id || !form.report_name) {
      setFormError('Portfolio and report name are required.')
      return
    }
    setSaving(true)
    setFormError(null)
    try {
      ...
      await saveSimPortfolioReport(payload, editRow?.id || null)
      setShowModal(false)
  ...
  async function handleDelete() {
    ...
    setDeleting(true)
    try {
      await deleteSimPortfolioReport(deleteTarget.id)
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-white">Practice Portfolio Reports</h1>
          <p className="text-gray-400 text-sm mt-1">All reports across all practice portfolios</p>
        </div>
        <div className="flex items-center gap-2">
          <ExportListMenu data={filtered} columns={exportColumns} baseFilename="sim-portfolio-reports" />
          <button
            onClick={openAdd}
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-lg p-6 space-y-4 max-h-screen overflow-y-auto">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-semibold text-lg">
                {editRow ? 'Edit Report' : 'Create Report'}
              </h2>
              <button
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  value={form.practice_portfolio_id}
                  onChange={(e) =>
                    setForm((f) => ({ ...f, practice_portfolio_id: e.target.value }))
                  }
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Q2 2026 Portfolio Status"
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, generation_status: e.target.value }))
                    }
                    className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm focus:outline-none"
                  >
                    {STATUSES.map((s) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-gray-400 text-sm mb-1">Report Date</label>
                  <input
                    type="date"
              ...
              <div>
                ...
                <textarea
                  value={form.report_summary}
                  onChange={(e) => setForm((f) => ({ ...f, report_summary: e.target.value }))}
                  rows={4}
                  className="w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-orange-500"
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-gray-800 border border-gray-700 rounded-xl w-full max-w-sm p-6 space-y-4">
            ...
            <p className="text-gray-400 text-sm">
              ...
              <strong className="text-white">{deleteTarget.report_name}</strong>?
            </p>
            <div className="flex justify-end gap-3">
              <button
                onClick={() => setDeleteTarget(null)}
                className="px-4 py-2 text-sm text-gray-300 bg-gray-700 rounded-lg"
              >
                Cancel
              </button>
              <button
