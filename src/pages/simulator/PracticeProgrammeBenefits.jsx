/**
 * Practice Programme Benefits (Simulator)
 * Mirrors Platform Programme Benefits: search + table of programmes, select one, show benefits.
 * Route: /simulator/practice-programme/benefits
 */
...
export default function PracticeProgrammeBenefitsPage() {
  ...
  const loadBenefits = async (programmeId) => {
    try {
      ...
      setError(err.message || 'Failed to load programme benefits')
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-gray-100">Programme Benefits</h1>
          </div>
          <p className="text-gray-400">
            Track benefit realization across practice programmes (from project benefits review plans).
          </p>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg border border-gray-700 p-4 mb-6">
          ...
          {listLoading ? (
            <div className="text-center py-8 text-gray-400 text-sm">Loading programmes...</div>
                  ...
                  {filteredProgrammes.map((p) => (
                    <tr
                      ...
                      className={`border-b border-gray-700/80 last:border-0 cursor-pointer transition-colors ${
                        selectedProgrammeId === p.id ? 'bg-blue-900/30 text-blue-100' : 'hover:bg-gray-700/50 text-gray-200'
                      }`}
                    >
          ...
          <p className="mt-2 text-xs text-gray-500">Click a row to select a programme and view its benefits below.</p>
        </div>

        {selectedProgrammeId && (
          <div className="mb-4">
            <h2 className="text-lg font-semibold text-gray-200 mb-3">Benefits for selected programme</h2>
            {benefitsLoading ? (
              ...
              <>
                ...
                <div className="mt-6 bg-gray-800 rounded-lg border border-gray-700 p-6 overflow-x-auto">
                  <table className="min-w-full text-sm">
                    <thead>
                    ...
                    <tbody>
                      {benefits.map((b) => (
                        <tr key={b.id} className="border-b border-gray-800 last:border-0">
                          ...
                          <td className="py-2 pr-4 text-gray-300 capitalize">{b.benefit_status || 'unknown'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
