...
import { Phone, Plus, Search, Calendar, Clock, Pencil, Trash2, X, Save } from 'lucide-react'
import { simDb } from '../../../../services/supabase/supabaseClient'
import { getCalls, createCall, updateCall, deleteCall } from '../../../../services/sim/simCommunicationsService'
...
const EMPTY_FORM = {
  ...
  join_link: '', platform_name: '', status: 'scheduled', notes: '',
}
...
export default function SimVoiceCallsPage() {
  ...
  useEffect(() => {
    simDb.auth.getUser().then(({ data }) => setUserId(data?.user?.id || null))
  }, [])
  ...
  const handleUpdate = async (form) => {
    ...
    try {
      ...
      setEditingId(null); setEditingCall(null)
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 mb-4 mt-4">
          <h1 className="text-xl font-semibold text-white flex items-center gap-2">
            <Phone className="h-5 w-5 text-blue-400" /> Voice Calls
          </h1>
          {projectId && (
        ...
        {loading && <div className="flex justify-center py-12"><div className="h-8 w-8 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" /></div>}
        ...
        <div className="space-y-3">
          {filtered.map(call => (
            <div key={call.id}>
              {editingId === call.id ? (
                <CallForm initial={editingCall} onSave={handleUpdate}
                  onCancel={() => { setEditingId(null); setEditingCall(null) }} saving={saving} />
              ) : (
                <div className="rounded-xl border border-slate-700 bg-slate-800 p-4">
                  ...
                  <div className="flex flex-wrap gap-4 text-xs text-slate-400 mb-3">
                    {call.scheduled_at && <span className="flex items-center gap-1"><Calendar className="h-3.5 w-3.5" /> {formatScheduled(call.scheduled_at)}</span>}
                    {call.duration_minutes && <span className="flex items-center gap-1"><Clock className="h-3.5 w-3.5" /> {call.duration_minutes} min</span>}
                    {call.join_link && <span className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" /> {call.join_link}</span>}
                  </div>
                  ...
                  <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-700">
                    <button type="button" onClick={() => { setEditingId(call.id); setEditingCall(call); setShowForm(false) }}
                      className="text-slate-400 hover:text-slate-200"><Pencil className="h-4 w-4" /></button>
