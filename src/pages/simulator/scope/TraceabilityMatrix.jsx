...
import { simListTraceability, simSaveTraceabilityRow, simSoftDeleteTraceRow } from '../../../services/sim/simPlanningService'
import { simListWbsNodes } from '../../../services/sim/simPlanningService'
...
export default function TraceabilityMatrix() {
  ...
  const load = useCallback(async () => {
    if (!projectId) return
    setLoading(true)
    const [t, w] = await Promise.all([simListTraceability(projectId), simListWbsNodes(projectId)])
    if (t.success) {
      setRows(t.data || [])
  ...
  const save = async () => {
    ...
    const res = await simSaveTraceabilityRow({ ...form, id: editing !== 'new' ? editing : undefined })
    if (res.success) {
      setEditing(null)
      load()
    }
  ...
  const del = async (id) => {
    ...
    const res = await simSoftDeleteTraceRow(id)
    if (res.success) load()
  }
