...
export default function AdminRoleMenuManagement() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (cancelled) return
        setCap(c)
        if (!canAccessAdminRoleMenuPage(c)) setDenied(true)
      } catch {
  ...
  return (
    <div>
      ...
      <RoleMenuCustomiser variant="admin" />
    </div>
  )
}
