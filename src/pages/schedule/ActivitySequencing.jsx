...
import { listDependencies, saveDependency, softDeleteDependency } from '../../services/activityDependencyService'
import { listActivities } from '../../services/activityListService'
...
export default function ActivitySequencing() {
  ...
  const load = useCallback(async () => {
    if (!projectId) return
    setLoading(true)
    const [d, a] = await Promise.all([listDependencies(projectId), listActivities(projectId)])
  ...
  const addDep = async () => {
    ...
    const res = await saveDependency(projectId, {
  ...
  const remove = async (id) => {
    ...
    const res = await softDeleteDependency(id, projectId)
    if (res.success) load()
  }
