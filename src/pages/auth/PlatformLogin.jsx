...
// Lazy load heavy components to prevent blocking initial render
const SSOLoginButton = lazy(() => import('../../components/security/SSOLoginButton'))
...
function PlatformLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        let hasPlatform = platformAccessResult.success && platformAccessResult.platforms && 
                          platformAccessResult.platforms.some(p => p.platform === 'platform' && p.has_registered)
        ...
        if (!hasPlatform) {
          try {
            ...
            // Always try to register for Platform if access is missing
            // This handles cases where registration didn't complete or record wasn't created
            console.log('Platform access not found, attempting to register for Platform...')
            try {
              await registerForPlatform(result.user.id, PLATFORMS.PLATFORM)
              console.log('Successfully registered for Platform during login recovery')
            } catch (registerError) {
              console.error('Error registering for Platform:', registerError)
              // Registration failed - user needs to register for Platform
              setError('You do not have access to the Platform. Please register for Platform access first.')
              setLoading(false)
              return
            }
            ...
            hasPlatform = platformAccessResult.success && platformAccessResult.platforms && 
                          platformAccessResult.platforms.some(p => p.platform === 'platform' && p.has_registered)
          } catch (recoveryError) {
        ...
        // Verify user has Platform access after recovery attempts
        if (!hasPlatform) {
          setError('You do not have access to the Platform. Please register for Platform access first.')
          setLoading(false)
          return
  ...
  const handleSSOLoginStart = useCallback(() => {
    setSsoLoading(true)
    setError(null)
  }, [])

  const handleSSOLoginComplete = useCallback(async (error, data) => {
    setSsoLoading(false)
    ...
    if (data?.user) {
      try {
        ...
        if (!hasPlatform) {
          try {
            console.log('Platform access not found for SSO user, attempting to register...')
  ...
  const handleClosePlatformSelector = useCallback(() => {
...
export default memo(PlatformLogin)
