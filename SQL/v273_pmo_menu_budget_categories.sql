-- ================================================
-- File: v273_pmo_menu_budget_categories.sql
-- Description: Add "Budget Categories" to PMO Admin sidebar menu.
...
BEGIN
  ...
  IF v_pmo_admin_id IS NULL THEN
    RAISE NOTICE 'pmo_admin_section not found – skipping Budget Categories menu item';
    RETURN;
  END IF;
  ...
  VALUES (
    ...
    'Budget Categories',
    'Manage budget category labels for project financial controls',
    v_pmo_admin_id,
    2,
    60,
    '/platform/pmo-admin/budget-categories',
    'clipboard-list',
    true,
    true
  )
  ...
  SELECT id INTO v_menu_id FROM menu_items WHERE menu_code = 'pmo_admin_budget_categories' LIMIT 1;
  ...
  RAISE NOTICE 'Budget Categories menu item added to PMO Admin';
END $$;
