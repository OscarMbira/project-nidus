-- =============================================
-- v256: Stakeholder Roles Lookup Table
-- Description: Create stakeholder_roles table for searchable dropdown
-- =============================================

-- Create stakeholder_roles table
CREATE TABLE IF NOT EXISTS stakeholder_roles (
    ...
    CONSTRAINT stakeholder_roles_name_unique UNIQUE (role_name)
);

-- Create index for faster lookups
CREATE INDEX IF NOT EXISTS idx_stakeholder_roles_active ON stakeholder_roles(is_active) WHERE is_deleted = false;
CREATE INDEX IF NOT EXISTS idx_stakeholder_roles_category ON stakeholder_roles(role_category) WHERE is_deleted = false;
CREATE INDEX IF NOT EXISTS idx_stakeholder_roles_display_order ON stakeholder_roles(display_order) WHERE is_deleted = false;
...
CREATE POLICY "stakeholder_roles_read_authenticated" ON stakeholder_roles
    FOR SELECT
    TO authenticated
    USING (true);
...
-- Seed initial stakeholder roles data
INSERT INTO stakeholder_roles (role_name, role_code, role_category, description, display_order, is_active) VALUES
-- Executive roles
...
VALUES
    ('stakeholder_roles', 'Lookup table for stakeholder roles used in project mandates and other documents', false, true)
...
COMMENT ON TABLE stakeholder_roles IS 'Lookup table for stakeholder roles - used in searchable dropdowns';
