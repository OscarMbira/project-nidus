-- v619: Make accept_project_invitation idempotent.
-- Previously the function returned FALSE when invitation_status was already
-- 'accepted', causing the UI to show "Failed to accept invitation" when the
-- same user re-clicked their invitation link after a partial earlier attempt.
-- Now: if the invitation was already accepted by the SAME user, the function
-- treats it as a success and returns TRUE without re-running the inserts.
...
BEGIN
  ...
  -- Idempotent re-acceptance: same user hitting accept a second time
  v_is_reaccept := (
...
COMMENT ON FUNCTION accept_project_invitation(VARCHAR, UUID) IS
  'Accepts invitation; creates user_roles row and project_memberships. '
  'Idempotent: returns TRUE without re-running inserts if the same user '
  'calls this function after the invitation is already accepted.';
