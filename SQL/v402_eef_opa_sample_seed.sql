...
BEGIN
  ...
  IF v_org_id IS NULL THEN
    RAISE NOTICE 'v402: No active account found — skip EEF/OPA sample seed.';
    RETURN;
  END IF;
  ...
  IF v_creator IS NULL THEN
    RAISE NOTICE 'v402: No user with auth_user_id — skip EEF/OPA sample seed.';
    RETURN;
  END IF;

  IF NOT EXISTS (SELECT 1 FROM auth.users au WHERE au.id = v_creator) THEN
    RAISE NOTICE 'v402: auth.users row missing for resolved creator — skip.';
    RETURN;
  END IF;
  ...
  -- --------------------------------------------------------------------------
  -- Platform: Enterprise Environment Factors
  -- --------------------------------------------------------------------------
  IF v_eef_count = 0 THEN
    ...
    RAISE NOTICE 'v402: Inserted sample EEF rows for account %.', v_org_id;
  ELSE
    RAISE NOTICE 'v402: EEF rows already exist for account % — skip EEF seed.', v_org_id;
  END IF;

  -- --------------------------------------------------------------------------
  -- Platform: Organisational Process Assets
  -- --------------------------------------------------------------------------
  IF v_opa_count = 0 THEN
    ...
    RAISE NOTICE 'v402: Inserted sample OPA rows for account %.', v_org_id;
  ELSE
    RAISE NOTICE 'v402: OPA rows already exist for account % — skip OPA seed.', v_org_id;
  END IF;

  -- --------------------------------------------------------------------------
  -- Simulator (sim schema) — same org + creator, separate idempotency
  -- --------------------------------------------------------------------------
  ...
  IF v_eef_count = 0 THEN
    ...
    RAISE NOTICE 'v402: Mirrored sample EEF into sim schema for account %.', v_org_id;
  ELSE
    RAISE NOTICE 'v402: Sim EEF already present for account % — skip sim EEF seed.', v_org_id;
  END IF;
  ...
  IF v_opa_count = 0 THEN
    ...
    RAISE NOTICE 'v402: Mirrored sample OPA into sim schema for account %.', v_org_id;
  ELSE
    RAISE NOTICE 'v402: Sim OPA already present for account % — skip sim OPA seed.', v_org_id;
  END IF;
END $$;
