-- =============================================================================
-- v778: sync_global_template_node — optional p_target (platform | simulator | both)
-- Allows Admin to publish Global Template Library rows to one app at a time.
-- Prerequisites: v777 (current sync entry point)
-- =============================================================================

-- Drop prior 3-arg catalog overloads (replaced with target-aware signatures)
DROP FUNCTION IF EXISTS public._sync_global_form_template_catalog(UUID, JSONB, TEXT);
DROP FUNCTION IF EXISTS public._sync_global_opa_catalog(UUID, JSONB, TEXT);
DROP FUNCTION IF EXISTS public._sync_global_process_template_catalog(UUID, JSONB, TEXT);

-- form_template catalog: honour target
...
BEGIN
    ...
    ELSE
        SELECT id INTO v_id FROM public.form_templates WHERE template_code = v_code LIMIT 1;
        SELECT COALESCE(MAX(version_number), 1) INTO v_ver
        FROM public.form_template_versions WHERE template_id = v_id;
        IF v_ver IS NULL THEN v_ver := 1; END IF;
    END IF;

    IF v_target IN ('simulator', 'both')
       AND EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'sim' AND table_name = 'form_templates')
    THEN
...
BEGIN
    ...
    IF v_target IN ('simulator', 'both')
       AND EXISTS (
           ...
           WHERE table_schema = 'sim' AND table_name = 'organisational_process_assets'
       )
    THEN
        IF v_id IS NULL THEN
            v_id := gen_random_uuid();
...
-- process_template catalog: honour target
...
-- Drop prior signatures so the new optional target resolves cleanly
DROP FUNCTION IF EXISTS public.sync_global_template_node(UUID, TEXT, TEXT, TEXT, TEXT, INTEGER, JSONB);
DROP FUNCTION IF EXISTS sim.sync_global_template_node(UUID, TEXT, TEXT, TEXT, TEXT, INTEGER, JSONB);
...
AS $$
    SELECT public.sync_global_template_node(
        ...
        p_category, p_version, p_payload, p_target
    );
$$;

REVOKE ALL ON FUNCTION public.sync_global_template_node(UUID, TEXT, TEXT, TEXT, TEXT, INTEGER, JSONB, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION sim.sync_global_template_node(UUID, TEXT, TEXT, TEXT, TEXT, INTEGER, JSONB, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.sync_global_template_node(UUID, TEXT, TEXT, TEXT, TEXT, INTEGER, JSONB, TEXT) TO service_role;
GRANT EXECUTE ON FUNCTION sim.sync_global_template_node(UUID, TEXT, TEXT, TEXT, TEXT, INTEGER, JSONB, TEXT) TO service_role;

DO $$
BEGIN
  RAISE NOTICE 'v778_sync_global_template_target.sql applied';
END $$;
