-- ============================================================================
-- v578: SECURITY DEFINER RPC to save email_configurations (bypasses RLS hang)
--
-- Symptom: Admin → Email Settings → Update Configuration times out after 12s
--   because PostgREST upsert waits on can_manage_email_configurations() per row
--   under RLS WITH CHECK (slow or never completes for some auth profiles).
--
-- Fix: Single transaction — deactivate sibling provider, upsert active row —
--   via save_email_configuration_as_admin() with SET row_security = off.
--
-- Prerequisites: v561/v563 (can_manage_email_configurations). Run in SQL Editor,
...
DO $$ BEGIN RAISE NOTICE 'v578_save_email_configuration_rpc.sql applied'; END $$;
