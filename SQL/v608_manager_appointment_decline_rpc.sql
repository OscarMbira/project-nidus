-- =============================================================================
-- v608_manager_appointment_decline_rpc.sql
-- Decline manager appointment + linked invitation (SECURITY DEFINER)
-- Prerequisites: v606
-- =============================================================================

CREATE OR REPLACE FUNCTION public.decline_manager_appointment(
...
BEGIN
  ...
  IF v_appt.appointee_user_id <> v_caller_user THEN
    RAISE EXCEPTION 'Only the appointee may decline this appointment' USING ERRCODE = '42501';
  END IF;
...
REVOKE ALL ON FUNCTION public.decline_manager_appointment(UUID, VARCHAR, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.decline_manager_appointment(UUID, VARCHAR, TEXT) TO authenticated;

COMMENT ON FUNCTION public.decline_manager_appointment IS
  'v608: Appointee declines manager appointment and linked invitation.';

DO $$ BEGIN RAISE NOTICE 'v608_manager_appointment_decline_rpc.sql applied'; END $$;
