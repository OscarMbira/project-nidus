-- =============================================================================
-- v783: Allow portfolio_template + programme_template on pm_template_* + sync RPC
-- Companion Admin: SQL/v184_portfolio_programme_templates.sql
-- Plan: project-nidus-admin/projectplans/v187_portfolio_programme_templates_plan.md
-- =============================================================================
...
ALTER TABLE public.pm_template_nodes
  ADD CONSTRAINT chk_pm_template_nodes_domain CHECK (
    domain IN (
      ...
      'portfolio_template', 'programme_template'
    )
  );
...
-- Recreate sync RPC with extended domain whitelist (body from v778 + new domains)
...
BEGIN
    ...
    ELSIF p_domain IN ('portfolio_template', 'programme_template') THEN
        -- Payload-only masters (no separate catalog table yet). Store object on pm_template_nodes.
        v_payload := COALESCE(p_payload, '{}'::jsonb);
        IF jsonb_typeof(v_payload) <> 'object' THEN
...
DO $$
BEGIN
  RAISE NOTICE 'v783_portfolio_programme_template_domains.sql applied';
END $$;
