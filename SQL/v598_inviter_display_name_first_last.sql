-- v598: Inviter display name — prefer first_name + last_name over handle-like full_name
-- Run after v587_validate_invitation_token_inviter_display_name.sql and v592_get_sent_invitations_rpc.sql
-- Fixes emails/tracker showing "oscarmbirablogging" when users.full_name matches email local part.

-- validate_invitation_token (accept page + token validation)
...
COMMENT ON FUNCTION public.validate_invitation_token(VARCHAR) IS
  'Validates invitation token; inviter_display_name prefers first+last over handle-like full_name.';

GRANT EXECUTE ON FUNCTION public.validate_invitation_token(VARCHAR) TO anon, authenticated, service_role;

-- get_sent_invitations_by_user (invitation tracker)
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'inviter_display_name', COALESCE(
        NULLIF(TRIM(CONCAT_WS(' ', NULLIF(TRIM(ub.first_name), ''), NULLIF(TRIM(ub.last_name), ''))), ''),
        CASE
          WHEN NULLIF(TRIM(ub.full_name), '') IS NOT NULL
           AND LOWER(TRIM(ub.full_name)) <> LOWER(SPLIT_PART(COALESCE(NULLIF(TRIM(ub.email), ''), '@'), '@', 1))
          THEN NULLIF(TRIM(ub.full_name), '')
        END,
        NULLIF(TRIM(ub.full_name), ''),
        NULLIF(TRIM(ub.email), '')
      ),
...
COMMENT ON FUNCTION public.get_sent_invitations_by_user(text, text, text, timestamptz, timestamptz) IS
  'Invitation tracker; inviter_display_name prefers first+last over handle-like full_name.';

NOTIFY pgrst, 'reload schema';

DO $$
BEGIN
  RAISE NOTICE 'v598_inviter_display_name_first_last.sql applied';
END $$;
